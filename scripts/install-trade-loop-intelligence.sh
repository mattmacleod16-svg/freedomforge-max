...
if [[ ! -f "$REPO_DIR/scripts/trade-loop-intelligence-maintain.sh" ]]; then
  echo "Missing $REPO_DIR/scripts/trade-loop-intelligence-maintain.sh" >&2
  exit 1
fi
...
TIMER_NAME="freedomforge-trade-loop-intelligence.timer"
SERVICE_PATH="/etc/systemd/system/$SERVICE_NAME"
TIMER_PATH="/etc/systemd/system/$TIMER_NAME"

sudo tee "$SERVICE_PATH" >/dev/null <<EOF
[Unit]
Description=FreedomForge Trade Loop Intelligence Maintenance
After=network-online.target
Wants=network-online.target

[Service]
Type=oneshot
ExecStart=/bin/bash '$REPO_DIR/scripts/trade-loop-intelligence-maintain.sh' --repo-dir '$REPO_DIR' --user '$SERVICE_USER' --app-base-url '$APP_BASE_URL' --timeout-sec '$TIMEOUT_SEC'
EOF

sudo tee "$TIMER_PATH" >/dev/null <<EOF
[Unit]
Description=Schedule FreedomForge Intelligence Maintenance
...
echo "✅ Intelligence maintenance timer installed"
