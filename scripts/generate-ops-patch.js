...
function buildEnvPatch(envPatch) {
  ...
  lines.push('# Apply selectively to your Vercel/GitHub environment variables');
  lines.push('');
...
function buildReport(summary, patch) {
  ...
  lines.push('- Apply these to deployment environment variables (Vercel/GitHub), then monitor weekly summary deltas.');
  lines.push('');
  return lines.join('\n') + '\n';
}
