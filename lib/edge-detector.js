...
function getBrainWeights() {
  try {
    if (_brainModule) {
      const evolved = _brainModule.getEvolvedWeights();
...
function getBrainThresholds() {
  ...
  return { overboughtRsi: 70, oversoldRsi: 30, bbPercentBHigh: 0.9, bbPercentBLow: 0.1, bbSqueezeWidth: 0.02, bbHighVolWidth: 0.08, volumeMinRatio: 0.8, volumeSurgeRatio: 2.0 };
}
...
async function getCompositeSignal({ asset = 'BTC', quoteCurrency = 'USD' } = {}) {
  ...
  if (bb) {
    ...
    if (bb.width < (bt.bbSqueezeWidth || 0.02)) compositeScore *= 1.15; // squeeze bonus
    else if (bb.width > (bt.bbHighVolWidth || 0.08)) compositeScore *= 0.85; // high vol penalty
  }
