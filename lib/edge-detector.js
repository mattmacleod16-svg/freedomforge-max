...
async function multiTimeframeMomentum(asset) {
  ...
  const TF_WEIGHTS = { '1m': 0.05, '5m': 0.10, '15m': 0.20, '1h': 0.30, '4h': 0.35 };
  const results = [];
  const candleCache = {};
...
async function getCompositeSignal({ asset = 'BTC', quoteCurrency = 'USD', weightsOverride = null } = {}) {
  ...
  // 2b. MACD — momentum divergence detector
  ...
  if (macdVal) {
    ...
    if (macdVal.histogram > 0 && macdVal.histogram > macdVal.histogramPrev) {
      compositeScore += w.rsi * 0.3; // piggyback on RSI weight
    } else if (macdVal.histogram < 0 && macdVal.histogram < macdVal.histogramPrev) {
      compositeScore -= w.rsi * 0.3;
    }
    // MACD cross (leading signal)
    if (macdVal.macd > macdVal.signal && macdVal.macd > 0) {
      compositeScore += 0.02; // small bullish boost
    } else if (macdVal.macd < macdVal.signal && macdVal.macd < 0) {
      compositeScore -= 0.02;
    }
  }
  ...
  // Determine side
  if (compositeScore > 0.06) side = 'buy';
  else if (compositeScore < -0.06) side = 'sell';
  else side = 'neutral';
