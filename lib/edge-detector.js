...
let _kellySizer = null;
try { _kellySizer = require('./kelly-sizer'); } catch (e) {
  _log.warn('kelly-sizer not loaded, using simplified sizing', { err: e?.message });
}

// ─── Regime Transition Forecaster ─────────────────────────────────────────────
let _regimeForecaster = null;
try { _regimeForecaster = require('./regime-transition-forecaster'); } catch (e) {
  _log.warn('regime-transition-forecaster not loaded', { err: e?.message });
}

// ─── Cross-Asset Regime Engine ────────────────────────────────────────────────
let _crossAssetRegime = null;
try { _crossAssetRegime = require('./cross-asset-regime'); } catch (e) {
  _log.warn('cross-asset-regime not loaded', { err: e?.message });
}

let _mlPipeline = null;
try { _mlPipeline = require('./ml-pipeline'); } catch {}

let _nexusBrain = null;
try { _nexusBrain = require('./nexus-brain'); } catch {}


// ─── Drawdown Intelligence ─────────────────────────────────────────────────
let _ddIntel = null;
try { _ddIntel = require('./drawdown-intelligence'); } catch (e) {
  _log.warn('drawdown-intelligence not loaded', { err: e?.message });
}
...
// ─── Enhanced Indicators (Wave 5) ───────────────────────────────────────────

/**
 * On-Balance Volume (OBV) — cumulative volume weighted by price direction.
 * Rising OBV confirms bullish trend; divergence with price signals weakness.
 * @param {Array<{close: number, volume: number}>} candles
 * @returns {{ obv: number, obvEma: number, trend: string, divergence: string|null }}
 */
function onBalanceVolume(candles) {
  ...
  for (let i = 1; i < candles.length; i++) {
    ...
    obvSeries.push(obvSeries[i - 1] + dir * candles[i].volume);
  }

  const obvEmaArr = ema(obvSeries, 10);
  const currentObv = obvSeries[obvSeries.length - 1];
  const currentObvEma = obvEmaArr[obvEmaArr.length - 1];
  const obvTrend = currentObv > currentObvEma ? 'bullish' : 'bearish';

  // Divergence detection: price makes higher high but OBV doesn't (or vice versa)
  let divergence = null;
  if (candles.length >= 20) {
    const recentHalf = Math.floor(candles.length / 2);
    const priceHigh1 = Math.max(...candles.slice(0, recentHalf).map(c => c.close));
    const priceHigh2 = Math.max(...candles.slice(recentHalf).map(c => c.close));
    const obvHigh1 = Math.max(...obvSeries.slice(0, recentHalf));
    const obvHigh2 = Math.max(...obvSeries.slice(recentHalf));

    if (priceHigh2 > priceHigh1 && obvHigh2 < obvHigh1) {
      divergence = 'bearish_divergence'; // Price up, OBV down = weakness
    } else if (priceHigh2 < priceHigh1 && obvHigh2 > obvHigh1) {
      divergence = 'bullish_divergence'; // Price down, OBV up = accumulation
    }
  }

  return { obv: currentObv, obvEma: currentObvEma, trend: obvTrend, divergence };
}

/**
 * Fibonacci retracement levels from recent swing high/low.
 * Identifies key support/resistance zones for entry/exit decisions.
 * @param {Array<{high: number, low: number, close: number}>} candles
 * @param {number} [lookback=50] — candles to scan for swing high/low
 * @returns {{ swingHigh, swingLow, levels, nearestLevel, proximity }}
 */
function fibonacciLevels(candles, lookback = 50) {
  ...
  for (const r of ratios) {
    levels[r] = swingHigh - range * r; // retracement from high
  }
  ...
  for (const [ratio, price] of Object.entries(levels)) {
    ...
    if (dist < minDist) {
      minDist = dist;
      nearestLevel = { ratio: Number(ratio), price };
    }
  }

  const proximity = range > 0 ? minDist / range : 0; // How close to a fib level (0 = at level)

  return { swingHigh, swingLow, levels, nearestLevel, proximity };
}

/**
 * Cross-asset momentum — BTC as leading indicator for altcoins.
 * When BTC leads a move, altcoins often follow with a lag.
 * @param {string} asset — the asset we're evaluating
 * @param {object} candleCache — cached candles from multiTimeframeMomentum
 * @returns {Promise<{ btcLead: string, alignment: number, btcMomentum: number }>}
 */
async function crossAssetMomentum(asset, candleCache) {
  if (asset === 'BTC') return { btcLead: 'self', alignment: 1, btcMomentum: 0 };

  try {
    // Get BTC 1h candles
    let btcCandles;
    if (candleCache?.['btc_1h']) {
      btcCandles = candleCache['btc_1h'];
    } else {
      btcCandles = await getCandles('BTC', '1h', 30);
    }

    if (!btcCandles || btcCandles.length < 10) return null;

    const btcCloses = btcCandles.map(c => c.close);
    const btcReturn5 = (btcCloses[btcCloses.length - 1] - btcCloses[Math.max(0, btcCloses.length - 6)]) /
      btcCloses[Math.max(0, btcCloses.length - 6)];

    // Get asset candles from cache or fetch
    let assetCandles;
    if (candleCache?.['1h']) {
      assetCandles = candleCache['1h'];
    } else {
      assetCandles = await getCandles(asset, '1h', 30);
    }

    if (!assetCandles || assetCandles.length < 10) return null;

    const assetCloses = assetCandles.map(c => c.close);
    const assetReturn5 = (assetCloses[assetCloses.length - 1] - assetCloses[Math.max(0, assetCloses.length - 6)]) /
      assetCloses[Math.max(0, assetCloses.length - 6)];

    // BTC lead direction
    const btcLead = btcReturn5 > 0.005 ? 'bullish' : btcReturn5 < -0.005 ? 'bearish' : 'neutral';

    // Alignment: are BTC and altcoin moving in same direction?
    const sameDirection = (btcReturn5 > 0 && assetReturn5 > 0) || (btcReturn5 < 0 && assetReturn5 < 0);
    const alignment = sameDirection ? 1 : -1;

    return {
      btcLead,
      alignment,
      btcMomentum: Math.round(btcReturn5 * 10000) / 10000,
      assetMomentum: Math.round(assetReturn5 * 10000) / 10000,
    };
  } catch {
    return null;
  }
...
/**
 * MACD histogram divergence — detects when price makes a new extreme
 * but MACD histogram doesn't confirm (hidden or regular divergence).
 * @param {number[]} closes
 * @returns {{ type: string, strength: number }|null}
 */
function macdDivergence(closes) {
  if (closes.length < 60) return null;

  const macdResult = macd(closes);
  const macdResult2 = macd(closes.slice(0, -10)); // 10 bars ago
  if (!macdResult || !macdResult2) return null;

  const currentPrice = closes[closes.length - 1];
  const prevPrice = closes[closes.length - 11];

  // Regular bearish divergence: higher price, lower MACD histogram
  if (currentPrice > prevPrice && macdResult.histogram < macdResult2.histogram) {
    const strength = Math.min(1, Math.abs(macdResult2.histogram - macdResult.histogram) /
      (Math.abs(macdResult2.histogram) || 0.001));
    return { type: 'bearish_divergence', strength };
  }

  // Regular bullish divergence: lower price, higher MACD histogram
  if (currentPrice < prevPrice && macdResult.histogram > macdResult2.histogram) {
    const strength = Math.min(1, Math.abs(macdResult.histogram - macdResult2.histogram) /
      (Math.abs(macdResult2.histogram) || 0.001));
    return { type: 'bullish_divergence', strength };
  }
...
async function getCompositeSignal({ asset = 'BTC', quoteCurrency = 'USD', weightsOverride = null } = {}) {
  ...
  // ─── Final resolution ──────────────────────────────────────────────────────
  // 11. On-Balance Volume (OBV) — volume-weighted trend confirmation
  try {
    const obvResult = onBalanceVolume(hourCandles);
    components.obv = obvResult;
    if (obvResult) {
      // OBV trend confirms direction → small boost; OBV divergence → warning
      if (obvResult.trend === 'bullish' && compositeScore > 0) {
        compositeScore *= 1.05; // volume confirms bullish
      } else if (obvResult.trend === 'bearish' && compositeScore < 0) {
        compositeScore *= 1.05; // volume confirms bearish
      }

      // Divergence is a strong counter-signal
      if (obvResult.divergence === 'bearish_divergence' && compositeScore > 0) {
        compositeScore *= 0.85; // price up but volume declining = weakness
        components.obvWarning = 'bearish_vol_divergence';
      } else if (obvResult.divergence === 'bullish_divergence' && compositeScore < 0) {
        compositeScore *= 0.85; // price down but volume accumulating = potential bottom
        components.obvWarning = 'bullish_vol_divergence';
      }
    }
  } catch { /* non-fatal */ }

  // 12. Fibonacci proximity — support/resistance awareness
  try {
    const fibResult = fibonacciLevels(hourCandles, 50);
    components.fibonacci = fibResult;
    if (fibResult && fibResult.proximity < 0.03) {
      // Very close to a key Fibonacci level — potential support/resistance
      const fibRatio = fibResult.nearestLevel?.ratio;
      if (fibRatio === 0.618 || fibRatio === 0.382 || fibRatio === 0.5) {
        // Near golden ratio levels — these are strong S/R zones
        // If price is above fib level and we're bullish: good support
        // If price is approaching from below: potential resistance
        components.fibAlert = {
          level: fibRatio,
          price: fibResult.nearestLevel.price,
          proximity: fibResult.proximity,
        };
        // Reduce conviction slightly when sitting right at a key level (uncertainty)
        compositeScore *= 0.95;
      }
    }
  } catch { /* non-fatal */ }

  // 13. Cross-asset momentum — BTC leading indicator for altcoins
  try {
    const crossAsset = await crossAssetMomentum(asset, mtf.candleCache);
    components.crossAssetMomentum = crossAsset;
    if (crossAsset && asset !== 'BTC') {
      // BTC leads bullish + our altcoin signal is bullish = confirmation
      if (crossAsset.btcLead === 'bullish' && compositeScore > 0) {
        compositeScore *= 1.05;
      } else if (crossAsset.btcLead === 'bearish' && compositeScore < 0) {
        compositeScore *= 1.05;
      }
      // BTC diverging from altcoin signal = reduce conviction
      else if (crossAsset.alignment < 0 && Math.abs(crossAsset.btcMomentum) > 0.01) {
        compositeScore *= 0.92;
        components.crossAssetWarning = 'btc_divergence';
      }
    }
  } catch { /* non-fatal */ }

  // 14. MACD histogram divergence — hidden reversal signals
  try {
    const macdDiv = macdDivergence(hourCloses);
    components.macdDivergence = macdDiv;
    if (macdDiv) {
      if (macdDiv.type === 'bearish_divergence' && compositeScore > 0) {
        compositeScore *= Math.max(0.80, 1 - macdDiv.strength * 0.2);
        components.macdDivWarning = 'bearish_momentum_weakening';
      } else if (macdDiv.type === 'bullish_divergence' && compositeScore < 0) {
        compositeScore *= Math.max(0.80, 1 - macdDiv.strength * 0.2);
        components.macdDivWarning = 'bullish_momentum_building';
      }
    }
  } catch { /* non-fatal */ }

  // ─── 15. Regime Transition Forecast ─────────────────────────────────────────
  // If a regime change is likely imminent (>60% probability), reduce conviction.
  // Reason: entering a trade when regime is about to flip = high adverse excursion risk.
  try {
    if (_regimeForecaster && typeof _regimeForecaster.forecastTransition === 'function') {
      const busCtxNow = getSignalBusContext();
      const currentRegime = busCtxNow.regime?.mode || null;
      if (currentRegime) {
        const forecast = await Promise.race([
          _regimeForecaster.forecastTransition({ currentRegime, asset }),
          new Promise(res => setTimeout(() => res(null), 800)), // 800ms timeout
        ]);
        if (forecast) {
          components.regimeForecast = forecast;
          const transitionProb = forecast.probability || 0;
          const isSignalAlignedWithNextRegime = (
            (forecast.nextRegime === 'bullTrend' && compositeScore > 0) ||
            (forecast.nextRegime === 'bearTrend' && compositeScore < 0)
          );
          const isSignalContraNextRegime = (
            (forecast.nextRegime === 'bearTrend' && compositeScore > 0) ||
            (forecast.nextRegime === 'bullTrend' && compositeScore < 0) ||
            forecast.nextRegime === 'extremeVolatility'
          );

          if (isSignalContraNextRegime && transitionProb > 0.6) {
            // Regime about to flip against us — reduce conviction significantly
            const penaltyFactor = 0.6 + 0.4 * (1 - transitionProb); // 0.6 at 100%, 0.8 at 50%
            compositeScore *= penaltyFactor;
            components.regimeTransitionWarning = {
              currentRegime,
              nextRegime: forecast.nextRegime,
              probability: transitionProb,
              action: `conviction_reduced_${(1 - penaltyFactor * 100).toFixed(0)}pct`,
            };
            _log.info('regime transition warning: contra-regime signal, conviction reduced', {
              asset, current: currentRegime, next: forecast.nextRegime, prob: transitionProb,
            });
          } else if (isSignalAlignedWithNextRegime && transitionProb > 0.55) {
            // Next regime aligns with our signal = small boost
            const boostFactor = 1 + 0.08 * transitionProb;
            compositeScore *= boostFactor;
            components.regimeTransitionBoost = {
              currentRegime, nextRegime: forecast.nextRegime, probability: transitionProb,
              action: 'conviction_boosted',
            };
            _log.debug('regime transition boost: aligned with next regime', {
              asset, next: forecast.nextRegime, prob: transitionProb,
            });
          }
        }
      }
    ...
    _log.debug('regime transition forecast skipped', { err: err?.message });
  }

  // ─── 16. Full Cross-Asset Regime Multiplier ───────────────────────────────
  // Use the dedicated cross-asset-regime engine (more sophisticated than #13's
  // simple BTC lead/lag check). Produces a consensus multiplier [0.5, 1.3].
  try {
    if (_crossAssetRegime && typeof _crossAssetRegime.crossAssetMultiplier === 'function') {
      const busCtxNow2 = getSignalBusContext();
      const btcRegime = busCtxNow2.regime?.mode || null;
      if (btcRegime) {
        const mult = await Promise.race([
          _crossAssetRegime.crossAssetMultiplier({ btcRegime, asset }),
          new Promise(res => setTimeout(() => res(null), 600)),
        ]);
        if (mult && typeof mult === 'number' && mult !== 1) {
          compositeScore *= mult;
          components.crossAssetRegimeMultiplier = mult;
          _log.debug('cross-asset regime multiplier applied', { asset, mult: mult.toFixed(3) });
        }
      }
    }
  } catch (err) {
    _log.debug('cross-asset regime skipped', { err: err?.message });
  }

  // ─── Original final resolution ────────────────────────────────────────────
  ...
  // 11. NEXUS Brain overlay — fuses microstructure data (funding, L/S ratio, OB imbalance, F&G)
  //     into the composite signal. This is the external market intelligence layer.
  if (_nexusBrain && typeof _nexusBrain.getSignalForAsset === 'function') {
    try {
      const nexusSig = _nexusBrain.getSignalForAsset(asset);
      if (nexusSig && nexusSig.confidence > 0.52 && (Date.now() - nexusSig.timestamp) < 10 * 60 * 1000) {
        const NEXUS_WEIGHT = 0.20; // NEXUS gets 20% blend weight
        const nexusDirection = nexusSig.side === 'buy' ? 1 : nexusSig.side === 'sell' ? -1 : 0;
        const nexusContribution = nexusDirection * (nexusSig.confidence - 0.50) * NEXUS_WEIGHT * 2;
        compositeScore += nexusContribution;
        components.nexusBrain = {
          side: nexusSig.side,
          confidence: nexusSig.confidence,
          fundingRate: nexusSig.components?.fundingRate,
          fearGreed: nexusSig.components?.fearGreed,
          lsRatio: nexusSig.components?.lsRatio,
          thesis: nexusSig.thesis,
          contribution: nexusContribution,
        };
        // Recalculate absScore after NEXUS injection
        const absScoreUpdated = Math.min(1, Math.abs(compositeScore));
        if (compositeScore > neutralBound) side = 'buy';
        else if (compositeScore < -neutralBound) side = 'sell';
        else side = 'neutral';
      }
    } catch { /* NEXUS failure is non-fatal */ }
  }
...
/**
 * Kelly Criterion position sizing — uses calibrated kelly-sizer when available,
 * falls back to simplified formula. Integrates regime, confidence, and account size.
 *
 * @param {object} signal   - Output of getCompositeSignal
 * @param {number} baseUsd  - Minimum order size in USD (fallback floor)
 * @param {number} maxMultiplier - Max scaling factor vs baseUsd (used in fallback)
 * @param {object} [opts]   - Extra options: { accountUsd, regime }
 * @returns {number} Adjusted USD order size
 */
function dynamicOrderSize(signal, baseUsd = 15, maxMultiplier = 3, opts = {}) {
  const { confidence = 0.5, edge = 0, components = {} } = signal;
  const regime = opts.regime || components.regime?.mode || null;
  const accountUsd = opts.accountUsd || Number(process.env.TRADING_ACCOUNT_USD || 1000);

  // ── Tier 1: Full calibrated Kelly (preferred) ──────────────────────────────
  if (_kellySizer && typeof _kellySizer.kellySize === 'function') {
    try {
      const { sizeUsd } = _kellySizer.kellySize({ accountUsd, confidence, regime, asset: signal.asset || 'BTC' });
      if (sizeUsd && sizeUsd > 0) {
        // Still cap at maxMultiplier × baseUsd as a hard safety limit
        const capped = Math.min(sizeUsd, baseUsd * maxMultiplier) * ddMultiplier;
        _log.debug(`dynamicOrderSize[kelly]: $${sizeUsd} → capped+dd $${capped.toFixed(2)} (account=$${accountUsd}, conf=${confidence.toFixed(2)}, regime=${regime}, ddMult=${ddMultiplier.toFixed(2)})`);
        return Math.round(capped * 100) / 100;
      }
    } catch (err) {
      _log.warn('kelly-sizer.kellySize failed, falling back', { err: err?.message });
    }
  }

  // ── Drawdown multiplier — scale down during losing streaks ────────────────
  const ddMultiplier = _ddIntel && typeof _ddIntel.getDrawdownMultiplier === 'function'
    ? _ddIntel.getDrawdownMultiplier()
    : 1.0;

  // Check halt condition
  if (_ddIntel && typeof _ddIntel.isHalted === 'function' && _ddIntel.isHalted()) {
    _log.warn('dynamicOrderSize: HALTED by drawdown intelligence — returning 0');
    return 0; // No trading during halt
  }

  // ── Tier 2: Simplified Kelly fallback ─────────────────────────────────────
  if (!edge || edge < 0.1) return baseUsd;
