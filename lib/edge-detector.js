...
function getSignalBusContext() {
  try {
    ...
    const virtueGuidance = bus.query({ type: 'virtue_guidance', maxAgeMs: 6 * 60 * 60 * 1000 }).slice(0, 1);
    const taxOptimization = bus.query({ type: 'tax_optimization', maxAgeMs: 12 * 60 * 60 * 1000 }).slice(0, 1);
    ...
    return {
      ...
      virtueGuidance: virtueGuidance[0]?.payload || null,
      virtueConfidence: virtueGuidance[0]?.confidence || 0,
      taxPreference: taxOptimization[0]?.payload?.venuePreference || null,
    };
  } catch {
...
async function getCompositeSignal({ asset = 'BTC', quoteCurrency = 'USD', weightsOverride = null } = {}) {
  ...
  // 15. UCFEE Virtue Guidance — virtue-aware confidence adjustment
  if (busCtx.available && busCtx.virtueGuidance) {
    const vg = busCtx.virtueGuidance;
    const vMult = vg.confidenceMultiplier || 1.0;
    if (vMult !== 1.0) {
      compositeScore *= vMult;
      components.virtueGuidance = {
        virtueIndex: vg.virtueIndex,
        mode: vg.mode,
        multiplier: vMult,
        positivityIndex: vg.positivityIndex,
      };
    }
    // Tax-efficient venue preference from UCFEE cross-agent intel
    if (vg.crossAgentIntel?.taxPreference && vg.crossAgentIntel.taxPreference !== 'neutral') {
      components.taxPreference = vg.crossAgentIntel.taxPreference;
    }
  }
