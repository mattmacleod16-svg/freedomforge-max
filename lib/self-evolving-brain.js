...
let _log;
function log() { return _log || (_log = createLogger('self-evolving-brain')); }
...
const LEARNING_RATE = Math.max(0.01, Math.min(0.3, Number(process.env.BRAIN_LEARNING_RATE || 0.05)));
const DECAY_RATE = Math.max(0.9, Math.min(0.999, Number(process.env.BRAIN_DECAY_RATE || 0.999)));
// Baseline: 50% random win rate contributes 0.3 (0.5*0.6), PF=1.0 contributes 0.133 (1/3*0.4) = 0.433
const ATTRIBUTION_BASELINE = 0.433;
...
const DEFAULT_THRESHOLDS = {
  ...
  regimeBoostFactor: 0.15,       // how much regime alignment boosts score
  regimePenaltyFactor: 0.10,     // how much contra-regime penalizes score
  // Forecast/geo/sentiment multipliers
  forecastBoostBps: 0.03,        // per-forecast alignment boost
  geoRiskDamper: 0.8,            // score multiplier when geo risk > 0.7
  contrarianBoost: 0.08,         // sentiment-momentum divergence boost
  sentimentConfirmFactor: 0.5,   // factor for confirming sentiment
  sentimentContraPenalty: 0.3,   // penalty factor for contra-sentiment
};
...
function loadBrainState() {
  ...
  return {
    ...
    regimeWeights: raw.regimeWeights || {},
...
function computeIndicatorAttribution(trades) {
  ...
  for (const key of Object.keys(DEFAULT_WEIGHTS)) {
    attribution[key] = { winTrades: 0, lossTrades: 0, totalTrades: 0, winPnl: 0, lossPnl: 0, totalSlippage: 0, slippageCount: 0 };
  }
  ...
  for (const trade of trades) {
    ...
    const slipPct = Number.isFinite(trade.slippagePct) ? Math.abs(trade.slippagePct) : 0;
    const weight = trade._recencyWeight || 1;

    // Helper to update an indicator's attribution (weighted by recency)
    function attr(key) {
      attribution[key].totalTrades += weight;
      if (slipPct > 0) { attribution[key].totalSlippage += slipPct * weight; attribution[key].slippageCount += weight; }
      if (isWin) { attribution[key].winTrades += weight; attribution[key].winPnl += (trade.pnl || 0) * weight; }
      else { attribution[key].lossTrades += weight; attribution[key].lossPnl += (trade.pnl || 0) * weight; }
    }

    // Multi-TF Momentum
    if (components.mtfConfluence > 0) attr('multiTfMomentum');
    // RSI
    if (components.rsi != null) attr('rsi');
    // Bollinger Bands
    if (components.bbPercentB != null) attr('bollingerBands');
    // Volume
    if (components.volRatio != null) attr('volumeConfirmation');
    // Regime alignment
    if (components.regime) attr('regimeAlignment');
    // Sentiment divergence
    if (components.sentDivergence) attr('sentimentDivergence');
  }
  ...
  for (const [key, attr] of Object.entries(attribution)) {
    ...
    // Average slippage for this indicator's trades
    attr.avgSlippage = attr.slippageCount > 0 ? Math.round(attr.totalSlippage / attr.slippageCount * 100) / 100 : 0;
    // Score: combine win rate, profit factor, and slippage penalty
    // High slippage (>0.5%) penalizes the score: each 0.1% over 0.5% reduces score by 0.02
    const slippagePenalty = attr.avgSlippage > 0.5 ? Math.min(0.15, (attr.avgSlippage - 0.5) * 0.2) : 0;
    attr.score = Math.max(0, winRate * 0.6 + Math.min(3, profitFactor) / 3 * 0.4 - slippagePenalty);
  }
...
function evolveWeights(currentWeights, attribution, learningRate = LEARNING_RATE) {
  ...
  for (const key of keys) {
    ...
    // Gradient: how much better than random baseline is this indicator?
    const gradient = (attr.score - ATTRIBUTION_BASELINE) * 2;
  ...
  // Soft-normalize: allow total to float between 0.8 and 1.2
  const total = Object.values(newWeights).reduce((s, v) => s + v, 0);
  if (total < 0.9 || total > 1.1) {
    const target = total < 0.9 ? 0.9 : 1.1;
    const factor = target / total;
    for (const k in newWeights) newWeights[k] *= factor;
  }
 ...
 */
const CONFIDENCE_CEILING = Math.max(0.50, Math.min(0.95, Number(process.env.BRAIN_CONFIDENCE_CEILING || 0.64)));
const CONFIDENCE_FLOOR = Math.max(0.30, Math.min(0.70, Number(process.env.BRAIN_CONFIDENCE_FLOOR || 0.50)));
...
function evolveThresholds(currentThresholds, calibration, assetProfiles, streaks) {
  ...
  } catch (err) { log().error('[brain] drought threshold adjustment error:', { error: err?.message || err }); }

  // Losing streak protection
  if (streaks.current < -(t.losingStreakThreshold || 5)) {
    t.minConfidence = Math.min(CONFIDENCE_CEILING, t.minConfidence + 0.05);
...
function runEvolutionCycle() {
  ...
  // Apply recency weighting — recent trades matter more
  const now = Date.now();
  const HALF_LIFE_MS = 14 * 24 * 60 * 60 * 1000; // 14-day half-life
  for (const trade of closedTrades) {
    const age = now - (trade.closedAt || trade.ts || now);
    trade._recencyWeight = Math.pow(0.5, age / HALF_LIFE_MS);
  }
  ...
  // 7. Evolve weights (with generation-decayed learning rate)
  const genNumber = state.totalEvolutions || 0;
  const effectiveLR = LEARNING_RATE * Math.pow(0.999, Math.min(genNumber, 500));
  const prevWeights = { ...state.weights };
  const newWeights = evolveWeights(state.weights, attribution, effectiveLR);
  ...
  // 9b. Evolve per-regime weights (lower threshold + Bayesian interpolation)
  const regimeWeights = {};
  const regimeMinTrades = Math.max(3, Math.floor(MIN_TRADES_FOR_LEARNING / 2));
  for (const [regime, profile] of Object.entries(regimeProfiles)) {
    if (profile.trades >= regimeMinTrades) {
      const regimeTrades = closedTrades.filter(t => (t.signalComponents?.regime || 'unknown') === regime);
      const regimeAttribution = computeIndicatorAttribution(regimeTrades);
      regimeWeights[regime] = evolveWeights(state.weights, regimeAttribution, effectiveLR);
      // Bayesian prior: blend global weights with regime-specific weights based on confidence
      const confidence = Math.min(1, profile.trades / MIN_TRADES_FOR_LEARNING);
      for (const key of Object.keys(regimeWeights[regime])) {
        regimeWeights[regime][key] = newWeights[key] * (1 - confidence) + regimeWeights[regime][key] * confidence;
        // NO additional decay here — blend handles regularization
        regimeWeights[regime][key] = Math.round(regimeWeights[regime][key] * 10000) / 10000;
      }
    }
  }
  ...
  // 10b. UCFEE-2.0: Virtue alignment validation — ensure evolved weights don't degrade virtue scores
  let virtueAlignmentCheck = null;
  try {
    const virtue = require('./virtue-engine');
    const preVirtueState = virtue.getVirtueIndex();
    const preVi = preVirtueState.virtueIndex;

    // Temporarily apply new weights and evaluate virtue impact
    const ctx = virtue.buildContextFromSystem();
    const postEval = virtue.evaluate(ctx);
    const postVi = postEval.virtueIndex;

    const viDelta = postVi - preVi;
    const degraded = viDelta < -0.05; // > 5% drop = rollback weights

    virtueAlignmentCheck = {
      preVirtueIndex: Math.round(preVi * 1000) / 1000,
      postVirtueIndex: Math.round(postVi * 1000) / 1000,
      delta: Math.round(viDelta * 1000) / 1000,
      degraded,
    };

    if (degraded) {
      log().warn('[brain] Virtue alignment degraded — rolling back weight evolution', {
        preVi: (preVi * 100).toFixed(1) + '%',
        postVi: (postVi * 100).toFixed(1) + '%',
      });
      // Revert to previous weights — virtue alignment takes priority
      Object.assign(newWeights, prevWeights);
      Object.assign(newThresholds, prevThresholds);
      generation.virtueRollback = true;
      generation.virtueRollbackReason = `Virtue Index dropped ${(Math.abs(viDelta) * 100).toFixed(1)}% (${(preVi * 100).toFixed(1)}% → ${(postVi * 100).toFixed(1)}%)`;
    }

    // Record virtue alignment in generation
    generation.virtueAlignment = virtueAlignmentCheck;

    // Push feedback to telemetry
    try {
      const telem = require('./telemetry-collector');
      telem.recordFeedback({
        signalType: 'brain_evolution',
        decisionId: `gen-${generation.id}`,
        outcome: degraded ? 'virtue_rollback' : 'aligned',
        beforeMetric: preVi,
        afterMetric: postVi,
        learningSource: 'virtue_alignment_check',
      });
    } catch {}
  } catch (virtueErr) {
    log().warn('[brain] Virtue engine unavailable for alignment check:', { error: virtueErr?.message || virtueErr });
  }
  ...
  state.regimeWeights = regimeWeights;
  ...
  } catch (err) { log().error('[brain] signal bus publish error:', { error: err?.message || err }); }
 ...
 */
function getEvolvedWeights(asset = null, regime = null) {
  const state = loadBrainState();
  if (regime && state.regimeWeights?.[regime]) {
    return { ...state.regimeWeights[regime] };
  }
