...
function computeIndicatorAttribution(trades) {
  ...
  for (const trade of trades) {
    ...
    // Helper to update an indicator's attribution
    function attr(key) {
      attribution[key].totalTrades++;
      if (slipPct > 0) { attribution[key].totalSlippage += slipPct; attribution[key].slippageCount++; }
      if (isWin) { attribution[key].winTrades++; attribution[key].winPnl += trade.pnl || 0; }
      else { attribution[key].lossTrades++; attribution[key].lossPnl += trade.pnl || 0; }
    }
...
function runEvolutionCycle() {
  ...
  // 9b. Evolve per-regime weights
  const regimeWeights = {};
  for (const [regime, profile] of Object.entries(regimeProfiles)) {
    if (profile.trades >= MIN_TRADES_FOR_LEARNING) {
      ...
      regimeWeights[regime] = evolveWeights(state.weights, regimeAttribution);
