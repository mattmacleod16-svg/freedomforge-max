...
const LEARNING_RATE = Math.max(0.01, Math.min(0.3, Number(process.env.BRAIN_LEARNING_RATE || 0.03)));
const DECAY_RATE = Math.max(0.9, Math.min(0.999, Number(process.env.BRAIN_DECAY_RATE || 0.997)));
...
function evolveWeights(currentWeights, attribution, learningRate = LEARNING_RATE) {
  ...
  if (total < 0.8 || total > 1.2) {
    const target = total < 0.8 ? 0.8 : 1.2;
...
function runEvolutionCycle() {
  ...
  const effectiveLR = LEARNING_RATE * Math.pow(0.998, Math.min(genNumber, 500));
  ...
  for (const [regime, profile] of Object.entries(regimeProfiles)) {
    if (profile.trades >= regimeMinTrades) {
      ...
      for (const key of Object.keys(regimeWeights[regime])) {
        ...
        regimeWeights[regime][key] = regimeWeights[regime][key] * DECAY_RATE + DEFAULT_WEIGHTS[key] * (1 - DECAY_RATE);
