...
const log = createLogger('websocket-feed');
...
class WebSocketFeed extends EventEmitter {
  ...
  async connect() {
    ...
    log.info(`Connected to ${connected}/2 exchanges`);
    ...
    return new Promise((resolve, reject) => {
      try {
        ...
        this.krakenWs.on('close', (code, reason) => {
          log.info(`Kraken WS closed: ${code} ${reason}`);
          this.krakenAlive = false;
  ...
  _resetKrakenHeartbeat() {
    ...
    this._krakenHeartbeat = setTimeout(() => {
      log.warn('Kraken heartbeat timeout — reconnecting');
      this.krakenWs?.terminate();
  ...
  _scheduleKrakenReconnect() {
    ...
    log.info(`Kraken reconnect in ${delay}ms (attempt ${this.krakenReconnectAttempts})`);
    ...
    return new Promise((resolve, reject) => {
      try {
        ...
        this.coinbaseWs.on('open', () => {
          ...
          if (apiKey && apiSecret) {
            try {
              ...
              log.warn('Coinbase signing failed, connecting unauthenticated:', signErr.message);
            }
          }
        ...
        this.coinbaseWs.on('close', (code, reason) => {
          log.info(`Coinbase WS closed: ${code} ${reason}`);
          this.coinbaseAlive = false;
  ...
  _resetCoinbaseHeartbeat() {
    ...
    this._coinbaseHeartbeat = setTimeout(() => {
      log.warn('Coinbase heartbeat timeout — reconnecting');
      this.coinbaseWs?.terminate();
    }, HEARTBEAT_TIMEOUT_MS);
  }
  ...
  _scheduleCoinbaseReconnect() {
    ...
    log.info(`Coinbase reconnect in ${delay}ms (attempt ${this.coinbaseReconnectAttempts})`);
