...
const EXCHANGE_CONFIGS = {
  ...
  nonkyc: {
    rateLimitKey: 'exchange:nonkyc',
    circuitKey:   'exchange:nonkyc',
    rateLimit:    { maxTokens: 10, refillPerSec: 5 },
    circuit:      { failureThreshold: 5, resetTimeMs: 120000 },
  },
};
...
/**
 * @param {object} opts — NonKyc.io privacy exchange client
 * @param {string} [opts.baseUrl='https://api.nonkyc.io/api/v2']
 */
function createNonKycClient(opts = {}) {
  const client = createExchangeClient('nonkyc', opts);
  return {
    fetchJson: client.fetchJson,
    getStatus: client.getStatus,
    baseUrl: opts.baseUrl || 'https://api.nonkyc.io/api/v2',
  };
}
