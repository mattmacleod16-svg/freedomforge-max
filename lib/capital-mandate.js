...
function checkMandate({ usdSize, confidence = 0.5, edge = 0, asset = '', venue = '' }) {
  ...
  if (mode === 'normal') {
    ...
    } catch (err) { console.error('[capital-mandate] NORMAL daily loss check error:', err?.message || err); }
  ...
  if (mode === 'growth') {
    ...
    } catch (err) { console.error('[capital-mandate] GROWTH daily loss check error:', err?.message || err); }
