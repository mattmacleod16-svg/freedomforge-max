...
// PERF: Persist hedge exposure state to survive restarts
const HEDGE_STATE_FILE = path.resolve(process.cwd(), process.env.HEDGE_STATE_FILE || 'data/hedge-state.json');

function _saveHedgeState() {
  try {
    const state = {};
    for (const [asset, entry] of exposureMap.entries()) {
      state[asset] = { longUsd: entry.longUsd, shortUsd: entry.shortUsd, netUsd: entry.netUsd };
    }
    if (rio) { rio.writeJsonAtomic(HEDGE_STATE_FILE, { exposure: state, stats, savedAt: Date.now() }); }
    else { fs.writeFileSync(HEDGE_STATE_FILE, JSON.stringify({ exposure: state, stats, savedAt: Date.now() }, null, 2)); }
  } catch { /* best-effort */ }
}

function _restoreHedgeState() {
  try {
    let data;
    if (rio) { data = rio.readJsonSafe(HEDGE_STATE_FILE, { fallback: null }); }
    else if (fs.existsSync(HEDGE_STATE_FILE)) { data = JSON.parse(fs.readFileSync(HEDGE_STATE_FILE, 'utf8')); }
    if (data?.exposure) {
      for (const [asset, entry] of Object.entries(data.exposure)) {
        exposureMap.set(asset, { longUsd: entry.longUsd || 0, shortUsd: entry.shortUsd || 0, netUsd: entry.netUsd || 0, positions: [] });
      }
    }
