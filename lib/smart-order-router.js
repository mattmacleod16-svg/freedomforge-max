...
const PRICE_WEIGHT = Math.max(0, Math.min(1, Number(process.env.ROUTER_PRICE_WEIGHT || 0.4)));
const HEALTH_WEIGHT = Math.max(0, Math.min(1, Number(process.env.ROUTER_HEALTH_WEIGHT || 0.25)));
const FILL_WEIGHT = Math.max(0, Math.min(1, Number(process.env.ROUTER_FILL_WEIGHT || 0.2)));
const LATENCY_WEIGHT = Math.max(0, Math.min(1, Number(process.env.ROUTER_LATENCY_WEIGHT || 0.15)));
...
function scoreVenue(venue, prices, side) {
  ...
  return {
    ...
    feeBps: VENUE_FEES_BPS[venue] || 30,
