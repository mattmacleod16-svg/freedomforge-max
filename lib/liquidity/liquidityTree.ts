...
export class LiquidityTree {
  ...
  private getNode(index: number): TreeNode {
    return this.nodes.get(index) ?? { amount: BigInt(0), lazy: BigInt(0), updateId: 0 };
  }
  ...
  private pushDown(index: number): void {
    ...
    if (childBaseSum === BigInt(0)) {
      // Nothing to distribute — clear lazy to avoid drift
      this.setNode(index, { ...node, lazy: BigInt(0) });
      return;
    }
  ...
  private rangeBaseSum(
    ...
    if (ql > nr || qr < nl) return BigInt(0);
  ...
  private rangeAddLazy(
    ...
    if (current.amount === BigInt(0)) return; // No base deposits in this subtree
  ...
  addLiquidity(amount: bigint): number {
    if (amount <= BigInt(0)) throw new RangeError('amount must be positive');
  ...
  distributePnl(amount: bigint, snapshotLeaf: number): void {
    ...
    if (snapshotBaseTotal === BigInt(0)) return;
  ...
  withdraw(leaf: number): bigint {
    ...
    this.setNode(leafIdx, { amount: BigInt(0), lazy: BigInt(0), updateId: node.updateId });
    this.updateAncestors(leafIdx);
  ...
  getBalance(leaf: number): bigint {
    if (leaf < 1 || leaf >= this.nextLeaf) return BigInt(0);
    ...
    for (const nodeIdx of path) {
      virtualLazy.set(nodeIdx, BigInt(0));
    }
    virtualLazy.set(leafIdx, BigInt(0));
    ...
    for (const nodeIdx of path) {
      ...
      const myVirtualLazy = virtualLazy.get(nodeIdx) ?? BigInt(0);
      const effectiveLazy = node.lazy + myVirtualLazy;

      if (effectiveLazy === BigInt(0)) continue;
      ...
      const leftVirtual = virtualLazy.get(leftIdx) ?? BigInt(0);
      const rightVirtual = virtualLazy.get(rightIdx) ?? BigInt(0);

      const childBaseSum = left.amount + right.amount;
      if (childBaseSum === BigInt(0)) continue;
    ...
    const leafVirtualLazy = virtualLazy.get(leafIdx) ?? BigInt(0);
  ...
  getStats(): LiquidityTreeStats {
    ...
    for (let leaf = 1; leaf < this.nextLeaf; leaf++) {
      ...
      if (balance > BigInt(0)) activeLeaves++;
    }
