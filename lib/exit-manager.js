...
// This state is ephemeral — if the process restarts, trails reset to entry price,
// which is safe (slightly less optimal but never dangerous).
...
async function placeExitOrder(trade, currentPrice, reason) {
  ...
  try {
    ...
    // For resilience, fall back to mark-to-market close so the journal still updates
    // and the brain can learn. The actual position may still be open on the exchange.
    return {
      success: true,
      exitPrice: currentPrice,
      fees,
...
async function placeCoinbaseExit(trade, exitSide, currentPrice, limitPrice) {
  ...
  if (cdpMode) {
    ...
    const payload = exitSide === 'sell'
      ? {
          ...
          side: 'SELL',
          order_configuration: { market_market_ioc: { base_size: String(roundDown(baseSize, 8)) } },
        }
      : {
          client_order_id: clientOrderId,
          product_id: productId,
          side: 'BUY',
          order_configuration: { market_market_ioc: { quote_size: String(trade.usdSize.toFixed(2)) } },
        };
  ...
  const bodyObj = exitSide === 'sell'
    ? { type: 'market', side: 'sell', product_id: productId, size: String(roundDown(baseSize, 8)) }
    : { type: 'market', side: 'buy', product_id: productId, funds: String(trade.usdSize.toFixed(2)) };
