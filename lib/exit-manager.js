    ...
    for (const trade of openPositions) {
      try {
        ...
        if (exitDecision.shouldExit) {
          const exitFraction = exitDecision.partial || 1.0;
          const isPartial = exitFraction < 1.0;
          const exitResult = await executeExit(trade, currentPrice, exitDecision.reason, exitDecision);
          if (exitResult.success) {
            if (isPartial) {
              // Partial exit: reduce position size but keep trade open
              try {
                const closedUsd = (trade.usdSize || 0) * exitFraction;
                const remainingUsd = (trade.usdSize || 0) - closedUsd;
                if (tradeJournal && typeof tradeJournal.updateTradeById === 'function') {
                  tradeJournal.updateTradeById(trade.id, {
                    usdSize: remainingUsd,
                ...
                _log.info('partial exit recorded', {
                  tradeId: trade.id, fraction: exitFraction,
                  closedUsd: closedUsd.toFixed(2), remainingUsd: remainingUsd.toFixed(2),
                });
              } catch (pErr) {
                _log.warn('failed to update partial exit in journal', {
                  tradeId: trade.id, error: pErr?.message,
                });
              }
            }
            result.exited++;
            result.exits.push({
              ...
              pnl: exitResult.pnl,
              reason: exitDecision.reason,
              partial: isPartial ? exitFraction : undefined,
            });
          } else {
            result.errors++;
          }
...
// ── Configuration: Time-based exits + Scale-out ──────────────────────────────
const MAX_HOLDING_HOURS = Math.max(1, Number(process.env.EXIT_MAX_HOLDING_HOURS || 48)); // 48h default
const STALE_POSITION_HOURS = Math.max(1, Number(process.env.EXIT_STALE_HOURS || 72)); // Force-exit stale
const SCALE_OUT_ENABLED = process.env.EXIT_SCALE_OUT_ENABLED !== 'false'; // Default: enabled
const SCALE_OUT_FIRST_TARGET_ATR = Number(process.env.EXIT_SCALE_OUT_FIRST_ATR || 2.0); // First partial at 2x ATR
const SCALE_OUT_FIRST_PCT = Number(process.env.EXIT_SCALE_OUT_FIRST_PCT || 0.50); // Close 50% at first target
const SCALE_OUT_SECOND_TARGET_ATR = Number(process.env.EXIT_SCALE_OUT_SECOND_ATR || 3.5); // Second partial at 3.5x ATR
const SCALE_OUT_SECOND_PCT = Number(process.env.EXIT_SCALE_OUT_SECOND_PCT || 0.30); // Close 30% at second target
// Remaining 20% rides with trailing stop

// Track scale-out state per trade: tradeId → { scaledOut1: bool, scaledOut2: bool, remainingPct: number }
const scaleOutState = new Map();

/**
 * Evaluate whether a position should be exited.
 * Uses trailing stop from risk-manager and fixed SL/TP as fallback.
 * NEW: Includes time-based exits and partial scale-out logic.
 *
 * @param {object} trade - Trade record from journal
 * @param {number} currentPrice - Current market price
 * @param {number|null} atr - Current ATR value (null if unavailable)
 * @returns {{ shouldExit: boolean, reason: string, trailingStop?: number, takeProfit?: number, partial?: number }}
 */
function evaluateExit(trade, currentPrice, atr) {
  ...
  if (trade.openedAt || trade.createdAt || trade.entryTime) {
    const openTime = new Date(trade.openedAt || trade.createdAt || trade.entryTime).getTime();
    const holdingMs = Date.now() - openTime;
    const holdingHours = holdingMs / (1000 * 60 * 60);

    // Force-exit stale positions (72h+ default)
    if (holdingHours >= STALE_POSITION_HOURS) {
      _log.warn('force-exiting stale position', {
        tradeId, asset: trade.asset, holdingHours: holdingHours.toFixed(1),
      });
      return {
        shouldExit: true,
        reason: `stale_position: held ${holdingHours.toFixed(1)}h (max ${STALE_POSITION_HOURS}h)`,
        partial: 1.0, // Full exit
      };
    }

    // Time-decay exit: if held past MAX_HOLDING_HOURS and not profitable, exit
    if (holdingHours >= MAX_HOLDING_HOURS) {
      ...
      if (unrealizedPnl <= 0) {
        _log.info('time-based exit: unprofitable after max hold', {
          tradeId, asset: trade.asset, holdingHours: holdingHours.toFixed(1),
          unrealizedPnlPct: (unrealizedPnl * 100).toFixed(2),
        });
        return {
          shouldExit: true,
          reason: `time_exit: held ${holdingHours.toFixed(1)}h with ${(unrealizedPnl * 100).toFixed(2)}% P&L`,
          partial: 1.0,
        };
      }

      // Profitable but held too long: tighten trailing stop
      // (Don't force exit — just note it for the trailing stop adjustment)
      _log.debug('time-extended profitable position', {
        tradeId, holdingHours: holdingHours.toFixed(1), pnlPct: (unrealizedPnl * 100).toFixed(2),
      });
    }
  }

  // ══ PARTIAL SCALE-OUT ═══════════════════════════════════════════════
  if (SCALE_OUT_ENABLED && atr && atr > 0) {
    if (!scaleOutState.has(tradeId)) {
      scaleOutState.set(tradeId, { scaledOut1: false, scaledOut2: false, remainingPct: 1.0 });
    }
    const soState = scaleOutState.get(tradeId);

    const priceDelta = side === 'buy' ? currentPrice - entryPrice : entryPrice - currentPrice;
    const atrMultiple = priceDelta / atr;

    // First scale-out: at SCALE_OUT_FIRST_TARGET_ATR (default 2.0x ATR profit)
    if (!soState.scaledOut1 && atrMultiple >= SCALE_OUT_FIRST_TARGET_ATR) {
      soState.scaledOut1 = true;
      soState.remainingPct -= SCALE_OUT_FIRST_PCT;
      _log.info('scale-out 1 triggered', {
        tradeId, asset: trade.asset, atrMultiple: atrMultiple.toFixed(2),
        closePct: (SCALE_OUT_FIRST_PCT * 100).toFixed(0) + '%',
        remainingPct: (soState.remainingPct * 100).toFixed(0) + '%',
      });
      return {
        shouldExit: true,
        reason: `scale_out_1: ${(atrMultiple).toFixed(1)}x ATR profit — closing ${(SCALE_OUT_FIRST_PCT * 100).toFixed(0)}%`,
        partial: SCALE_OUT_FIRST_PCT,
      };
    }

    // Second scale-out: at SCALE_OUT_SECOND_TARGET_ATR (default 3.5x ATR profit)
    if (soState.scaledOut1 && !soState.scaledOut2 && atrMultiple >= SCALE_OUT_SECOND_TARGET_ATR) {
      soState.scaledOut2 = true;
      soState.remainingPct -= SCALE_OUT_SECOND_PCT;
      _log.info('scale-out 2 triggered', {
        tradeId, asset: trade.asset, atrMultiple: atrMultiple.toFixed(2),
        closePct: (SCALE_OUT_SECOND_PCT * 100).toFixed(0) + '%',
        remainingPct: (soState.remainingPct * 100).toFixed(0) + '%',
      });
      return {
        shouldExit: true,
        reason: `scale_out_2: ${(atrMultiple).toFixed(1)}x ATR profit — closing ${(SCALE_OUT_SECOND_PCT * 100).toFixed(0)}%`,
        partial: SCALE_OUT_SECOND_PCT,
      };
    }
  }
...
async function executeExit(trade, currentPrice, reason, exitDecision) {
  try {
    // Determine exit fraction for partial exits
    const exitFraction = (exitDecision && exitDecision.partial) || 1.0;

    // 1. Place exit order (with fractional sizing for partials)
    const tradeForOrder = exitFraction < 1.0
      ? { ...trade, usdSize: (trade.usdSize || 0) * exitFraction }
      : trade;
    const orderResult = await placeExitOrder(tradeForOrder, currentPrice, reason);
    ...
    // 2. Calculate P&L (proportional to exit fraction)
    const exitUsdSize = (trade.usdSize || 0) * exitFraction;
    const rawPnl = trade.side === 'buy'
      ? (exitPrice - trade.entryPrice) / trade.entryPrice * exitUsdSize
      : (trade.entryPrice - exitPrice) / trade.entryPrice * exitUsdSize;
    ...
    // 5. Clean up trailing state (only on full exits)
    if (exitFraction >= 1.0) {
      trailingState.delete(trade.id);
      scaleOutState.delete(trade.id);
    }
...
module.exports = {
  ...
  evaluateExit,
  getScaleOutState: () => Object.fromEntries(scaleOutState),
  clearScaleOutState: (tradeId) => { if (tradeId) scaleOutState.delete(tradeId); else scaleOutState.clear(); },
};
