 ...
 */
function recordReconciliation({ closedCount = 0, totalPnl = 0, closed = [], batchId = null } = {}) {
  const ledger = loadLedger();

  if (closedCount === 0 && closed.length === 0) return ledger;

  // FIX M-5: Idempotency guard — skip if this batch was already recorded
  if (!ledger._processedBatchIds) ledger._processedBatchIds = [];
  // Build batch ID: use explicit batchId, or derive from trade IDs (only if all trades have IDs)
  const tradeIds = closed.length > 0 ? closed.map(t => t.id).filter(Boolean) : [];
  const effectiveBatchId = batchId || (tradeIds.length === closed.length && tradeIds.length > 0
    ? tradeIds.sort().join(',')
    : null);
  if (effectiveBatchId && ledger._processedBatchIds.includes(effectiveBatchId)) {
    return ledger; // Already recorded — skip to prevent double-counting
  }
  if (effectiveBatchId) {
    ledger._processedBatchIds.push(effectiveBatchId);
    // Keep only last 200 batch IDs to prevent unbounded growth
    if (ledger._processedBatchIds.length > 200) ledger._processedBatchIds = ledger._processedBatchIds.slice(-200);
  }
