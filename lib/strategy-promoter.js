...
function getLivePerformance(strategyName) {
  ...
  try {
    // Get stats for the last 30 days; trade journal does not filter by strategy name
    // directly, so we return the aggregate stats as a proxy.
    return tradeJournal.getStats({ sinceDays: 30 });
  } catch (err) {
