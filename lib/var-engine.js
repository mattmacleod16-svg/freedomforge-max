...
// ══════════════════════════════════════════════════════════════════════════════
// NEW: Monte Carlo VaR Simulation
// ══════════════════════════════════════════════════════════════════════════════

/**
 * Monte Carlo VaR — simulate N portfolio return paths using bootstrapped
 * historical returns and fat-tailed adjustments. More accurate than
 * parametric VaR for crypto's non-normal return distributions.
 *
 * @param {number[]} returns    - Historical return series (%)
 * @param {Object}   [opts={}]
 * @param {number}   [opts.simulations=5000]      - Number of Monte Carlo paths
 * @param {number}   [opts.horizon=1]              - Holding period in days
 * @param {number}   [opts.confidence=0.95]        - Confidence level
 * @param {boolean}  [opts.fatTails=true]          - Apply fat-tail adjustment
 * @param {number}   [opts.portfolioValueUsd=0]    - If >0, return VaR in dollar terms
 * @returns {{
 *   mcVaR95: number, mcVaR99: number,
 *   mcCVaR95: number, mcCVaR99: number,
 *   skewness: number, kurtosis: number,
 *   fatTailMultiplier: number,
 *   simulations: number,
 *   percentiles: { p1: number, p5: number, p10: number, p25: number, p50: number }
 * }}
 */
function monteCarloVaR(returns, opts = {}) {
  const {
    simulations = 5000,
    horizon = 1,
    confidence = 0.95,
  ...
  if (!Array.isArray(returns) || returns.length < 10) {
    return {
      mcVaR95: 0, mcVaR99: 0, mcCVaR95: 0, mcCVaR99: 0,
      skewness: 0, kurtosis: 3, fatTailMultiplier: 1,
      simulations: 0, percentiles: { p1: 0, p5: 0, p10: 0, p25: 0, p50: 0 },
    };
  }
  ...
  const mu = mean(clean);
  const sigma = stdDev(clean);
  const n = clean.length;

  // ── Distribution moments ───────────────────────────────────────────
  const skewness = computeSkewness(clean, mu, sigma);
  const kurtosis = computeKurtosis(clean, mu, sigma);

  // Fat-tail multiplier: excess kurtosis increases tail risk
  // Normal distribution has kurtosis = 3; crypto is typically 5-15
  const excessKurtosis = kurtosis - 3;
  const fatTailMultiplier = fatTails ? 1 + Math.max(0, excessKurtosis) * 0.05 : 1;

  // ── Simulate N paths ──────────────────────────────────────────────
  // Bootstrap with replacement + optional Cornish-Fisher tail stretch
  const pathReturns = new Float64Array(simulations);

  for (let i = 0; i < simulations; i++) {
    let cumulativeReturn = 0;

    for (let d = 0; d < horizon; d++) {
      // Bootstrap: random sample from historical returns
      const idx = Math.floor(Math.random() * n);
      let r = clean[idx];

      // Fat-tail adjustment: stretch extreme returns by kurtosis factor
      if (fatTails && Math.abs(r - mu) > 1.5 * sigma) {
        const sign = r > mu ? 1 : -1;
        r = mu + sign * Math.abs(r - mu) * fatTailMultiplier;
      }

      cumulativeReturn += r;
    }

    pathReturns[i] = cumulativeReturn;
  }

  // ── Sort simulated returns ─────────────────────────────────────────
  const sorted = Array.from(pathReturns).sort((a, b) => a - b);

  // ── Extract percentiles ────────────────────────────────────────────
  const pctile = (p) => sorted[Math.max(0, Math.floor(sorted.length * p))];

  const mcVaR95 = pctile(0.05);
  const mcVaR99 = pctile(0.01);

  // CVaR: mean of tail below VaR
  const idx95 = Math.max(1, Math.floor(sorted.length * 0.05));
  const idx99 = Math.max(1, Math.floor(sorted.length * 0.01));
  const tail95 = sorted.slice(0, idx95);
  const tail99 = sorted.slice(0, idx99);
  const mcCVaR95 = mean(tail95);
  const mcCVaR99 = mean(tail99);

  const result = {
    mcVaR95: round6(mcVaR95),
    mcVaR99: round6(mcVaR99),
    mcCVaR95: round6(mcCVaR95),
    mcCVaR99: round6(mcCVaR99),
    skewness: round6(skewness),
    kurtosis: round6(kurtosis),
    fatTailMultiplier: round6(fatTailMultiplier),
    simulations,
    percentiles: {
      p1: round6(pctile(0.01)),
      p5: round6(pctile(0.05)),
      p10: round6(pctile(0.10)),
      p25: round6(pctile(0.25)),
      p50: round6(pctile(0.50)),
    },
  };

  // Optionally express in dollar terms
  if (portfolioValueUsd > 0) {
    result.dollarVaR95 = round4(Math.abs(mcVaR95 / 100) * portfolioValueUsd);
    result.dollarVaR99 = round4(Math.abs(mcVaR99 / 100) * portfolioValueUsd);
    result.dollarCVaR95 = round4(Math.abs(mcCVaR95 / 100) * portfolioValueUsd);
  }

  log.info('[var-engine] Monte Carlo VaR complete', {
    simulations, horizon, var95: result.mcVaR95, var99: result.mcVaR99,
    skew: result.skewness, kurt: result.kurtosis,
  });
...
// ══════════════════════════════════════════════════════════════════════════════
// NEW: Cornish-Fisher VaR (fat-tail adjusted parametric)
// ══════════════════════════════════════════════════════════════════════════════

/**
 * Cornish-Fisher expansion adjusts the standard normal Z-score for
 * skewness and excess kurtosis, giving a more accurate tail quantile
 * for non-normal distributions (which crypto always is).
 *
 * Adjusted Z = z + (z² - 1) * S/6 + (z³ - 3z) * K/24 - (2z³ - 5z) * S²/36
 *
 * @param {number[]} returns        - Historical return series
 * @param {number}   [confidence=0.95]
 * @param {number}   [holdingPeriodDays=1]
 * @returns {{ cfVaR: number, cfCVaR: number, adjustedZ: number, normalZ: number,
 *             skewness: number, kurtosis: number, tailRatio: number }}
 */
function cornishFisherVaR(returns, confidence = 0.95, holdingPeriodDays = 1) {
  if (!Array.isArray(returns) || returns.length < 10) {
    return { cfVaR: 0, cfCVaR: 0, adjustedZ: 0, normalZ: 0, skewness: 0, kurtosis: 3, tailRatio: 1 };
  }
  ...
  const mu = mean(clean);
  const sigma = stdDev(clean);
  const S = computeSkewness(clean, mu, sigma);
  const K = computeKurtosis(clean, mu, sigma) - 3; // Excess kurtosis
  ...
  const cfVaR = mu - adjustedZ * sigma * sqrtT;

  // CVaR approximation with Cornish-Fisher: increase the tail beyond VaR by ~20%
  const cfCVaR = cfVaR * 1.2;

  // Tail ratio: how much worse the fat-tail VaR is vs normal
  const normalVaR = mu - z * sigma * sqrtT;
  const tailRatio = normalVaR !== 0 ? cfVaR / normalVaR : 1;

  return {
    cfVaR: round6(cfVaR),
    cfCVaR: round6(cfCVaR),
    adjustedZ: round6(adjustedZ),
    normalZ: round6(z),
    skewness: round6(S),
    kurtosis: round6(K + 3), // Report full kurtosis, not excess
    tailRatio: round6(tailRatio),
  };
}

// ══════════════════════════════════════════════════════════════════════════════
// NEW: EWMA Volatility (Exponentially Weighted Moving Average)
// ══════════════════════════════════════════════════════════════════════════════

/**
 * EWMA volatility gives more weight to recent observations, reacting
 * faster to regime changes than simple sample stddev.
 * Uses lambda=0.94 (RiskMetrics standard) by default.
 *
 * σ²_t = λ * σ²_{t-1} + (1 - λ) * r²_{t-1}
 *
 * @param {number[]} returns   - Historical return series (most recent last)
 * @param {number}   [lambda=0.94]
 * @returns {{ ewmaVol: number, sampleVol: number, ratio: number, regimeShift: boolean }}
 */
function ewmaVolatility(returns, lambda = 0.94) {
  if (!Array.isArray(returns) || returns.length < 5) {
    return { ewmaVol: 0, sampleVol: 0, ratio: 1, regimeShift: false };
  }
  ...
  const m = mean(clean);
  let variance = clean.slice(0, Math.min(10, clean.length)).reduce((s, r) => s + (r - m) ** 2, 0) /
    Math.min(10, clean.length);
  ...
  const ewmaVol = Math.sqrt(Math.max(0, variance));
  const sampleVol = stdDev(clean);

  // Regime shift detector: EWMA >> sample = recent vol spike
  const ratio = sampleVol > 0 ? ewmaVol / sampleVol : 1;
  const regimeShift = ratio > 1.5 || ratio < 0.5; // >50% divergence = regime shift

  return {
    ewmaVol: round6(ewmaVol),
    sampleVol: round6(sampleVol),
    ratio: round6(ratio),
    regimeShift,
  };
}
...
// ══════════════════════════════════════════════════════════════════════════════
// NEW: Comprehensive VaR Dashboard
// ══════════════════════════════════════════════════════════════════════════════

/**
 * Run ALL VaR methodologies on a return series and return a unified report.
 * This is the go-to function for the maintenance workforce and risk checks.
 *
 * @param {number[]} returns
 * @param {Object}   [opts={}]
 * @param {number}   [opts.confidence=0.95]
 * @param {number}   [opts.horizon=1]
 * @param {number}   [opts.portfolioValueUsd=0]
 * @returns {{ historical, parametric, monteCarlo, cornishFisher, ewma, consensus }}
 */
function comprehensiveVaR(returns, opts = {}) {
  const { confidence = 0.95, horizon = 1, portfolioValueUsd = 0 } = opts;
  const clean = (returns || []).filter(r => Number.isFinite(r));
  ...
  // Run all 4 methods
  const historical = calculateVaR(clean, confidence);
  const parametric = parametricVaR(mu, sigma, confidence, horizon);
  const monteCarlo = monteCarloVaR(clean, { confidence, horizon, portfolioValueUsd });
  const cf = cornishFisherVaR(clean, confidence, horizon);
  const ewma = ewmaVolatility(clean);

  // Consensus VaR: take the most conservative (most negative) across methods
  const allVaR95 = [historical.var95, parametric.var95, monteCarlo.mcVaR95, cf.cfVaR].filter(v => v !== 0);
  const allVaR99 = [historical.var99, parametric.var99, monteCarlo.mcVaR99].filter(v => v !== 0);

  const consensusVaR95 = allVaR95.length > 0 ? Math.min(...allVaR95) : 0;
  const consensusVaR99 = allVaR99.length > 0 ? Math.min(...allVaR99) : 0;

  // Average VaR (for balanced estimate)
  const avgVaR95 = allVaR95.length > 0 ? mean(allVaR95) : 0;
  ...
  return {
    ...
    ewma,
    consensus: {
      conservativeVaR95: round6(consensusVaR95),
      conservativeVaR99: round6(consensusVaR99),
      averageVaR95: round6(avgVaR95),
      methodsUsed: allVaR95.length,
      recommendation: ewma.regimeShift
        ? 'REGIME SHIFT detected — use conservative VaR and reduce position sizes'
        : Math.abs(cf.tailRatio) > 1.3
          ? 'Fat tails detected — Cornish-Fisher VaR more accurate than parametric'
          : 'Normal conditions — parametric VaR acceptable',
    },
...
// ── Higher Moment Helpers ────────────────────────────────────────────────────

function computeSkewness(arr, mu, sigma) {
  if (arr.length < 3 || sigma === 0) return 0;
  const n = arr.length;
  const m3 = arr.reduce((s, v) => s + ((v - mu) / sigma) ** 3, 0) / n;
  return m3;
}

function computeKurtosis(arr, mu, sigma) {
  if (arr.length < 4 || sigma === 0) return 3; // Normal
  const n = arr.length;
