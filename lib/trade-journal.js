...
function recordOutcome(tradeId, { exitPrice, fillPrice, pnl, pnlPercent, fees = 0 } = {}) {
  ...
  // FIX H-4: Double-close guard — prevent second close from overwriting first
  if (trade.closedAt && trade.outcome && trade.outcome !== 'unknown') {
    return false; // Already closed — don't overwrite
  }
  ...
  // Tax classification — auto-tag for tax advisor agent
  try {
    const { classifyTrade } = require('../scripts/tax-advisor-agent');
    trade.taxClassification = classifyTrade(trade);
  } catch {
    // tax-advisor module not available — non-fatal
  }

  // Telemetry — record trade outcome metrics for continuous improvement
  try {
    const telem = require('./telemetry-collector');
    telem.counter('trades_closed', { venue: trade.venue, asset: trade.asset, outcome: trade.outcome, strategy: trade.strategy || 'unknown' });
    if (trade.pnl != null) telem.histogram('trade_pnl_usd', trade.pnl, { venue: trade.venue, side: trade.side });
    if (trade.fees) telem.histogram('trade_fees_usd', trade.fees, { venue: trade.venue });
    if (trade.slippagePct != null) telem.histogram('trade_slippage_pct', trade.slippagePct, { venue: trade.venue });
    if (trade.entryTs && trade.closedAt) {
      const holdMs = new Date(trade.closedAt).getTime() - trade.entryTs;
      telem.histogram('trade_hold_duration_ms', holdMs, { asset: trade.asset });
    }
    telem.recordDecisionQuality({
      predictedConfidence: trade.signal?.confidence || 0.5,
      predictedSide: trade.side,
      actualReturn: trade.pnlPercent || 0,
      venue: trade.venue || 'unknown',
      strategy: trade.strategy || 'unknown',
    });
  } catch {
    // telemetry module not available — non-fatal
  }
...
function getStats(options = {}) {
  const journal = loadJournal();
  const { sinceDays = 30, venue = null, asset = null, strategy = null } = options;
  ...
  const feeAdjustedPnl = totalPnl - totalFees;
  ...
  // Sharpe-like ratio (daily returns — uses pnl/entryValue, not absolute P&L)
  const dailyReturnData = {};
  for (const t of closed) {
    ...
    if (!dailyReturnData[day]) dailyReturnData[day] = { pnl: 0, capital: 0 };
    const feePnl = (t.pnl || 0) - (t.fees || 0);
    dailyReturnData[day].pnl += feePnl;
    dailyReturnData[day].capital += (t.usdSize || 0);
  }
  const dailyReturns = Object.values(dailyReturnData)
    .filter(d => d.capital > 0)
    .map(d => d.pnl / d.capital);
  ...
  // Per-strategy attribution (arb, edge, composite, momentum, etc.)
  const strategyAttribution = {};
  for (const t of closed) {
    const key = t.strategy || 'unknown';
    if (!strategyAttribution[key]) strategyAttribution[key] = { trades: 0, wins: 0, pnl: 0, fees: 0 };
    strategyAttribution[key].trades += 1;
    if (t.outcome === 'win') strategyAttribution[key].wins += 1;
    strategyAttribution[key].pnl += t.pnl || 0;
    strategyAttribution[key].fees += t.fees || 0;
  }
  for (const key of Object.keys(strategyAttribution)) {
    const s = strategyAttribution[key];
    s.feeAdjustedPnl = Math.round((s.pnl - s.fees) * 100) / 100;
    s.pnl = Math.round(s.pnl * 100) / 100;
  ...
  return {
    ...
    feeAdjustedPnl: Math.round(feeAdjustedPnl * 100) / 100,
