...
// Kelly position sizer for adaptive sizing feedback
let _kellySizer;
try { _kellySizer = require('./kelly-sizer'); } catch { _kellySizer = null; }
...
function recordOutcome(tradeId, { exitPrice, fillPrice, pnl, pnlPercent, fees = 0 } = {}) {
  ...
  // Feed outcome to Kelly calibrator for adaptive position sizing
  if (_kellySizer && typeof _kellySizer.recordOutcome === 'function' && trade.pnlPercent != null) {
    try {
      _kellySizer.recordOutcome({
        won:            trade.outcome === 'win',
        pnlPct:         trade.pnlPercent / 100,  // kelly expects decimal, not percent
        asset:          trade.asset || 'BTC',
        regime:         trade.signalComponents?.regime?.mode || null,
        confidence:     trade.signalComponents?.confidence || 0.5,
      ...
      // Non-fatal — kelly calibration is best-effort
    }
  }
