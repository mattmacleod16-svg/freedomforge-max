...
async function emergencyRetry(fn, label) {
  ...
  for (let attempt = 0; attempt < EMERGENCY_RETRY_ATTEMPTS; attempt++) {
    try {
      ...
      lastErr = err;
      console.error(`[GUARDIAN] ${label} attempt ${attempt + 1}/${EMERGENCY_RETRY_ATTEMPTS} FAILED: ${err.message}`);
...
async function emergencyCloseCoinbase(positions) {
  ...
  return emergencyRetry(async (attempt) => {
    ...
    if (attempt > 0) console.log(`[GUARDIAN] Retry ${attempt + 1} for emergency close ${worst.productId}`);
...
async function emergencyCloseKraken(positions) {
  ...
  return emergencyRetry(async (attempt) => {
    if (attempt > 0) console.log(`[GUARDIAN] Retry ${attempt + 1} for Kraken emergency close ${worst.pair}`);
...
async function runGuardianCycle() {
  ...
  if (cb.warnings.length > 0) {
    ...
    console.log(`[GUARDIAN] Coinbase: futures $${cb.futuresBalance.toFixed(2)} + spot $${cb.spotBalance.toFixed(2)} = $${cb.totalBalance.toFixed(2)} | margin ${cb.marginPct.toFixed(1)}% | ${cb.positions.length} positions | OK`);
  }
  ...
  } catch (err) { console.error('[guardian] signal bus publish error:', err?.message || err); }
  ...
  try {
    ...
    if (krakenCritical) {
      ...
      console.log(`[GUARDIAN] Kill switch ACTIVATED — Kraken margin critical`);
      ...
      if (cbOk && krOk) {
        ...
        console.log(`[GUARDIAN] Kill switch AUTO-CLEARED — conditions normalized (CB:${cb.marginPct.toFixed(1)}% KR:${kr.marginPct.toFixed(1)}%)`);
      }
    }
  } catch (err) { console.error('[guardian] kill switch logic error:', err?.message || err); }
