...
async function checkCoinbaseMargin() {
  ...
  try {
    ...
    // Calculate margin utilization (based on total balance including spot)
    if (result.totalBalance > 0 && result.initialMargin > 0) {
      result.marginPct = result.totalBalance > 0 ? (result.initialMargin / result.totalBalance) * 100 : 0;
    }
...
async function checkKrakenMargin() {
  ...
  try {
    ...
    if (result.marginLevel !== null && result.marginLevel < 180 && result.marginUsed > 0) {
      result.healthy = false;
      result.warnings.push(`DANGER: Kraken margin level at ${result.marginLevel.toFixed(1)}% (< 180% danger zone)`);
    }
...
async function emergencyCloseCoinbase(positions) {
  ...
  // Sort by largest notional value first (close biggest position to free most margin)
  const sorted = [...positions].sort((a, b) => {
    const notionalA = Math.abs((a.contracts || a.size || 0) * (a.currentPrice || a.markPrice || 0));
    const notionalB = Math.abs((b.contracts || b.size || 0) * (b.currentPrice || b.markPrice || 0));
    return notionalB - notionalA; // largest notional first
  });
...
function shouldAllowNewTrade(venue, opts = {}) {
  ...
  if (tradeType === 'spot' && venue === 'coinbase') {
    ...
    const freeForSpot = spotBal > 0 ? spotBal : 0; // spot trades must NEVER use futures margin
    if (freeForSpot < 5) {
...
async function runGuardianCycle() {
  ...
  try {
    ...
    const krakenCritical = kr.marginLevel !== null && kr.marginLevel < 150;
    if (krakenCritical) {
      riskManager.activateKillSwitch(`liquidation-guardian: KR margin level ${kr.marginLevel} < 150`);
