 ...
 *
 * When Redis is available (via redis-signal-bus.js), delegates to Redis for
 * faster, race-condition-free inter-agent communication. Falls back to JSON
 * file storage when Redis is not configured.
 *
 * Usage:
...
// ─── Redis Delegation ─────────────────────────────────────────────────────────
// If redis-signal-bus is available and SIGNAL_BUS_MODE is not 'file',
// delegate all operations to the Redis-backed implementation.
let redisBus;
try {
  if (String(process.env.SIGNAL_BUS_MODE || 'auto').toLowerCase() !== 'file') {
    redisBus = require('./redis-signal-bus');
  }
} catch { redisBus = null; }
...
const TYPE_TTL_DEFAULTS = {
  edge_opportunity: 5 * 60 * 1000,          // 5 minutes - edges expire fast
  arb_opportunity: 60 * 1000,               // 1 minute - arbs expire very fast
  market_regime: 30 * 60 * 1000,            // 30 minutes
  alpha_regime: 30 * 60 * 1000,             // 30 minutes
  sentiment: 60 * 60 * 1000,               // 1 hour
  forecast: 4 * 60 * 60 * 1000,            // 4 hours
  geo_risk: 6 * 60 * 60 * 1000,            // 6 hours
  brain_evolution: 6 * 60 * 60 * 1000,     // 6 hours
  asset_intelligence: 12 * 60 * 60 * 1000, // 12 hours
  promoted_weights: 30 * 60 * 1000,        // 30 minutes
  tax_optimization: 12 * 60 * 60 * 1000,  // 12 hours - tax advice refreshes twice daily
  virtue_guidance: 6 * 60 * 60 * 1000,    // 6 hours - UCFEE virtue-aware trade guidance
  user_empowerment: 12 * 60 * 60 * 1000,  // 12 hours - user-facing empowerment metrics
  system_improvement: 12 * 60 * 60 * 1000, // 12 hours - telemetry-driven improvement recommendations
};
...
function save(signals) {
  ...
  const tmp = SIGNAL_FILE + '.tmp.' + process.pid;
...
function publish(signal) {
  ...
  signals.push({
    ...
    ttlMs: Number(signal.ttlMs || TYPE_TTL_DEFAULTS[signal.type] || DEFAULT_TTL_MS),
    publishedAt: Date.now(),
  });

  // Keep only the most recent MAX_SIGNALS
  const trimmed = signals.length > MAX_SIGNALS ? signals.slice(-MAX_SIGNALS) : signals;
  save(trimmed);

  // Telemetry — track agent signal publishing for mesh health
  try {
    const telem = require('./telemetry-collector');
    telem.counter('signal_bus_publish', { type: signal.type, source: signal.source });
    telem.agentPing(signal.source, { latencyMs: 0 });
  } catch {
...
function query(filter = {}) {
  const raw = load();
  const signals = prune(raw);
  // Lazy purge: persist pruned store if expired signals were removed
  if (signals.length < raw.length) save(signals);
  const now = Date.now();
  return signals
    .filter((s) => {
