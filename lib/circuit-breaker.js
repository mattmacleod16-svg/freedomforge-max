...
function tripOpen(service, breaker, reason) {
  ...
  if (wasState !== STATE.OPEN) {
    log.info(`${service}: OPENED after ${breaker.failures} failures (cooldown: ${Math.round(breaker.cooldownMs / 1000)}s, trips: ${breaker.tripCount})`);
  }
}
...
function transitionClosed(service, breaker) {
  ...
  if (wasOpen) {
    breaker.cooldownMs = COOLDOWN_MS;
    log.info(`${service}: CLOSED — service recovered (was tripped ${breaker.tripCount} times)`);
...
function reset(service) {
  ...
  breaker.successes = 0;
  breaker.cooldownMs = COOLDOWN_MS;
  log.info(`${service}: manually reset to CLOSED`);
}
