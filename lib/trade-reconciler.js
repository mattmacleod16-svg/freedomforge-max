...
let tradeJournal, riskManager, rio, mlPipeline;
...
try { mlPipeline = require('./ml-pipeline'); } catch { mlPipeline = null; }

const { createLogger } = require('./logger');
const log = createLogger('trade-reconciler');
...
const DYNAMIC_SLTP_ENABLED = String(process.env.TRADE_DYNAMIC_SLTP || 'true').toLowerCase() !== 'false';
const ATR_SL_MULTIPLIER = Math.max(0.5, Number(process.env.TRADE_ATR_SL_MULT || 1.5));
const ATR_TP_MULTIPLIER = Math.max(1.0, Number(process.env.TRADE_ATR_TP_MULT || 2.0));
const ATR_MIN_SL_PCT = 0.5;
const ATR_MAX_SL_PCT = 8.0;
const ATR_MIN_TP_PCT = 1.0;
const ATR_MAX_TP_PCT = 12.0;
const ATR_CACHE_TTL_MS = 60 * 60 * 1000; // 1 hour
...
async function fetchCurrentPrice(asset) {
  ...
  const assetMap = {
    ...
    EGLD: { coingecko: 'elrond-erd-2', krakenPair: 'EGLDUSD' },
    MEX: { coingecko: 'maiar-dex', krakenPair: null },
    // Solana ecosystem extras
    JUP: { coingecko: 'jupiter-exchange-solana', krakenPair: 'JUPUSD' },
    RAY: { coingecko: 'raydium', krakenPair: 'RAYUSD' },
    MSOL: { coingecko: 'msol', krakenPair: null },
    // Equities (via Alpaca price, no crypto pair)
    SPY: { coingecko: null, krakenPair: null, alpaca: true },
    QQQ: { coingecko: null, krakenPair: null, alpaca: true },
    AAPL: { coingecko: null, krakenPair: null, alpaca: true },
    MSFT: { coingecko: null, krakenPair: null, alpaca: true },
    NVDA: { coingecko: null, krakenPair: null, alpaca: true },
    GOOGL: { coingecko: null, krakenPair: null, alpaca: true },
    AMZN: { coingecko: null, krakenPair: null, alpaca: true },
  };
...
// ─── ATR Computation (for dynamic SL/TP) ─────────────────────────────────────
...
/**
 * Fetch ATR as a percentage of current price from Kraken hourly OHLC.
 ...
 * @param {number} [periods=14] - ATR lookback periods
 * @returns {Promise<number|null>} ATR as percentage of price, or null
 */
async function fetchATRPercent(asset, currentPrice, periods = 14) {
  if (!currentPrice || currentPrice <= 0) return null;
  const upper = asset.toUpperCase();

  // Check cache
  const cached = atrCache.get(upper);
  ...
  try {
    ...
    const timer = setTimeout(() => controller.abort(), 10000);
    try {
      const res = await fetch(`https://api.kraken.com/0/public/OHLC?pair=${pair}&interval=60`, { signal: controller.signal });
      const data = await res.json();
      if (data.error?.length) return null;
      const key = Object.keys(data.result || {}).find(k => k !== 'last');
      if (!key) return null;
      const candles = data.result[key];
      if (!candles || candles.length < periods + 1) return null;
      ...
      const atr = atrSum / periods;
      const atrPct = (atr / currentPrice) * 100;
      atrCache.set(upper, { atrPct, fetchedAt: Date.now() });
      return atrPct;
    } finally { clearTimeout(timer); }
  } catch {
    return null;
...
async function reconcileOpenTrades() {
  ...
  // Fetch ATR percentages for dynamic SL/TP
  const atrMap = new Map();
  if (DYNAMIC_SLTP_ENABLED) {
    await Promise.allSettled(
      assets.map(async (asset) => {
        const price = prices.get(asset.toUpperCase());
        const atrPct = await fetchATRPercent(asset, price);
        if (atrPct !== null) atrMap.set(asset.toUpperCase(), atrPct);
      })
    );
  }
  ...
  for (const trade of toProcess) {
    ...
    if (isZombie || (!currentPrice && !trade.entryPrice)) {
      ...
      } catch (err) { log.error('zombie tracking error', { error: err?.message || err }); }
      continue;
    }
    ...
    // FIX H-1: Subtract estimated round-trip fees from P&L (maker+taker ~0.1% each side)
    const FEE_RATE = Number(process.env.RECONCILER_FEE_RATE || 0.002); // 0.2% round-trip default
    const feeCost = (trade.usdSize || 0) * FEE_RATE;
    const pnl = ((trade.usdSize || 0) * pnlPercent / 100) - feeCost;
    const holdTimeMs = now - (trade.entryTs || trade.ts || now);  // FIX H-5: default to now if missing, not 0

    // Compute dynamic SL/TP based on ATR (or fall back to fixed)
    let dynSL = SL_PERCENT;
    let dynTP = TP_PERCENT;
    if (DYNAMIC_SLTP_ENABLED) {
      const atrPct = atrMap.get(trade.asset?.toUpperCase());
      if (atrPct && atrPct > 0) {
        dynSL = -Math.min(ATR_MAX_SL_PCT, Math.max(ATR_MIN_SL_PCT, atrPct * ATR_SL_MULTIPLIER));
        dynTP = Math.min(ATR_MAX_TP_PCT, Math.max(ATR_MIN_TP_PCT, atrPct * ATR_TP_MULTIPLIER));
      }
    }
    ...
    // Rule 2: Take profit (ATR-scaled)
    if (pnlPercent >= dynTP) {
      shouldClose = true;
      closeReason = `take_profit (${pnlPercent.toFixed(2)}% >= ${dynTP.toFixed(2)}%)`;
    }

    // Rule 3: Stop loss (ATR-scaled)
    if (pnlPercent <= dynSL) {
      shouldClose = true;
      closeReason = `stop_loss (${pnlPercent.toFixed(2)}% <= ${dynSL.toFixed(2)}%)`;
    }
    ...
    try {
      ...
      if (success) {
        ...
        if (riskManager) {
          ...
          } catch (err) { log.error('risk manager update failed', { error: err?.message || err }); }
        }

        // ═══ ML ONLINE LEARNING — Feed outcome back into ML feature store ═══
        // Extracts features from the trade's signal components and records
        // win/loss, enabling automatic retraining when enough new samples accumulate.
        if (mlPipeline && typeof mlPipeline.recordOutcome === 'function' && typeof mlPipeline.extractFeatures === 'function') {
          try {
            const signal = trade.signal || {};
            const components = trade.signalComponents || {};
            const features = mlPipeline.extractFeatures(signal, components);
            const outcome = pnl > 0 ? 1 : 0; // 1 = win, 0 = loss
            mlPipeline.recordOutcome(
              Object.fromEntries(features.names.map((n, i) => [n, features.values[i]])),
              outcome
            );
          } catch (err) { log.error('ML outcome recording failed', { error: err?.message || err }); }
        }
      }
    } catch (e) {
  ...
  if (zombieIds.length > 0) {
    ...
    } catch (err) { log.error('zombie cleanup write failed', { error: err?.message || err }); }
