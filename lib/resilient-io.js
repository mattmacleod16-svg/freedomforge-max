...
// Lazy logger to avoid circular dependency (logger.js requires resilient-io)
let _log;
function getLog() {
  if (!_log) {
    try { _log = require('./logger').createLogger('resilient-io'); } catch { _log = null; }
  }
  return _log || { error: console.error.bind(console), warn: console.warn.bind(console), info: console.log.bind(console) };
}
...
function acquireLock(filePath, opts = {}) {
  ...
  while (Date.now() < deadline) {
    ...
    try {
      ...
      // Lock held by someone else — yield with setImmediate-like sleep (non-blocking)
      const spinMs = 10 + Math.random() * 40;
      const until = Date.now() + spinMs;
      // Use Atomics.wait on a shared buffer for non-blocking sleep
      // Fallback: brief busy wait is acceptable for <50ms in a sync context
      const sharedBuf = new Int32Array(new SharedArrayBuffer(4));
      try { Atomics.wait(sharedBuf, 0, 0, Math.ceil(spinMs)); } catch { while (Date.now() < until) { /* busy wait fallback */ } }
    }
  }

  // Timeout — force-break only if lock is stale, otherwise fail-safe
  try {
    const lockInfo = fs.existsSync(lp) ? JSON.parse(fs.readFileSync(lp, 'utf8')) : null;
    const lockAge = lockInfo ? Date.now() - (lockInfo.ts || 0) : staleMs + 1;
    if (lockAge < staleMs) {
      // Lock is fresh — another process is actively writing. Do NOT force-break.
      getLog().error('Lock held — refusing to force-break', { file: path.basename(filePath), holderPid: lockInfo?.pid, ageMs: lockAge });
      throw new Error(`Lock timeout on ${path.basename(filePath)} — active lock by pid=${lockInfo?.pid}`);
    }
    // Lock is stale — safe to break
    getLog().warn('Force-breaking stale lock', { file: path.basename(filePath), ageMs: lockAge, holderPid: lockInfo?.pid });
  } catch (readErr) {
    if (readErr.message.includes('Lock timeout')) throw readErr;
    // Can't read lock info — proceed with force break as last resort
    getLog().warn('Force-breaking unreadable lock', { file: path.basename(filePath) });
  }
  try { fs.unlinkSync(lp); } catch {}
  const fd = fs.openSync(lp, 'wx');
...
function readJsonSafe(filePath, opts = {}) {
  ...
  try {
    ...
    getLog().error('Primary read failed', { file: path.basename(abs), error: err.message });
  }
  ...
  for (let i = 0; i < MAX_BACKUPS; i++) {
    ...
    try {
      if (fs.existsSync(bakPath)) {
        ...
        getLog().warn('Recovered from backup', { file: path.basename(abs), backup: `.bak.${i}` });
        // Restore the backup as primary atomically (tmp + rename)
        const restoreTmp = abs + '.restore.' + process.pid;
        try {
          fs.writeFileSync(restoreTmp, JSON.stringify(data, null, 2));
          fs.renameSync(restoreTmp, abs);
        } catch (restoreErr) {
          try { fs.unlinkSync(restoreTmp); } catch {}
          getLog().error('Failed to restore backup as primary', { file: path.basename(abs), error: restoreErr.message });
        }
        return data;
      }
    } catch {}
...
async function rateLimitWait(key, opts = {}, maxWaitMs = 5000) {
  ...
  // Exceeded wait — throw instead of silently proceeding (protects exchange rate limits)
  throw new Error(`[resilient-io] Rate limit wait exceeded for "${key}" after ${maxWaitMs}ms`);
}
...
// Lightweight wrapper for scripts that just need a timeout on raw fetch.
// For retry + circuit breaker, use fetchRetry instead.

/**
 * fetch() with a hard timeout. Drop-in replacement.
 * @param {string} url
 * @param {object} [opts] — standard fetch options
 * @param {number} [timeoutMs=15000] — abort after this many ms
 * @returns {Promise<Response>}
 */
...
/**
 * Locked read-modify-write cycle for JSON state files.
 * Prevents race conditions when multiple timers touch the same file.
 * @param {string} filePath — path to JSON file
 * @param {function} mutator — (data) => mutatedData (sync)
 * @param {object} [fallback={}] — default if file missing/corrupt
 */
function lockedUpdate(filePath, mutator, fallback = {}) {
