...
function acquireLock(filePath, opts = {}) {
  ...
  try {
    ...
    if (lockAge < staleMs) {
      ...
      console.error(`[resilient-io] Lock on ${path.basename(filePath)} held by pid=${lockInfo?.pid} for ${lockAge}ms — refusing to force-break (data safety)`);
      throw new Error(`Lock timeout on ${path.basename(filePath)} — active lock by pid=${lockInfo?.pid}`);
    }
    // Lock is stale — safe to break
    console.warn(`[resilient-io] Force-breaking stale lock on ${path.basename(filePath)} (age=${lockAge}ms, holder pid=${lockInfo?.pid})`);
  } catch (readErr) {
    if (readErr.message.includes('Lock timeout')) throw readErr;
    // Can't read lock info — proceed with force break as last resort
    console.warn(`[resilient-io] Force-breaking unreadable lock on ${path.basename(filePath)}`);
  }
  try { fs.unlinkSync(lp); } catch {}
  const fd = fs.openSync(lp, 'wx');
    ...
    try {
      if (fs.existsSync(bakPath)) {
        ...
        try {
          ...
          console.error(`[resilient-io] Failed to restore backup as primary: ${restoreErr.message}`);
        }
        return data;
      }
    } catch {}
