...
function acquireLock(filePath, opts = {}) {
  ...
  while (Date.now() < deadline) {
    ...
    try {
      ...
      const release = () => {
        try { fs.unlinkSync(lp); } catch {}
        activeLocks.delete(absFilePath);
      };
      activeLocks.set(absFilePath, release);
      return release;
    } catch (err) {
...
function lockedUpdate(filePath, mutator, fallback = {}) {
  ...
  try {
    ...
    if (updated !== undefined) {
      // Pass lock: false — we already hold the lock; re-acquiring would deadlock
      writeJsonAtomic(filePath, updated, { lock: false });
    }
    return updated || data;
  } finally {
    release();
