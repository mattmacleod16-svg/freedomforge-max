...
const TASK_TYPES = {
  market_sentiment:    { primary: ['grok', 'gemini'],         fallback: ['groq', 'openai'] },
  risk_assessment:     { primary: ['anthropic', 'openai'],    fallback: ['mistral', 'gemini'] },
  trade_signal:        { primary: ['groq', 'cerebras'],       fallback: ['grok', 'nvidia'] },
  code_generation:     { primary: ['openai', 'anthropic'],    fallback: ['mistral', 'gemini'] },
  data_analysis:       { primary: ['nvidia', 'gemini'],       fallback: ['openai', 'cerebras'] },
  portfolio_review:    { primary: ['anthropic', 'openai'],    fallback: ['gemini', 'mistral'] },
  news_analysis:       { primary: ['grok', 'gemini'],         fallback: ['groq', 'mistral'] },
  ...
  large_context:       { primary: ['gemini', 'anthropic'],    fallback: ['openai', 'mistral'] },
  cost_sensitive:      { primary: ['groq', 'cerebras', 'gemini'], fallback: ['ollama-local', 'huggingface'] },
  consensus_needed:    { primary: ['openai', 'anthropic', 'grok'], fallback: ['gemini', 'mistral', 'groq'] },
  adversarial_check:   { primary: ['anthropic', 'openai'],    fallback: ['grok', 'mistral'] },
...
function getConsensusRoute(availableModels = []) {
  ...
  const consensusPool = ['openai', 'anthropic', 'grok', 'gemini', 'mistral', 'groq']
...
function getAdversarialRoute(availableModels = []) {
  const available = new Set(availableModels.map(m => m.toLowerCase()));
  const proposers = ['openai', 'grok', 'gemini'].filter(m => available.has(m));
