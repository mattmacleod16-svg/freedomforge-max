 ...
 *   1. Owner Payout (minimum 25% per capital mandate — sovereign law)
 *   2. API Operations Reserve (covers next 30 days of projected API costs)
 *   3. Gas Reserve (ETH for on-chain operations)
 *   4. Trading Capital Compounding (grow the portfolio)
 *
...
// Percentage of revenue directed to the Impact Fund wallet (Solana)
// Set IMPACT_FUND_WALLET to enable; defaults to 10%.
// Capped at 20% to ensure owner payout + operational reserves are never crowded out.
const IMPACT_FUND_PCT = Math.max(0, Math.min(20,
  Number(process.env.IMPACT_FUND_PCT || 10)));
const IMPACT_FUND_WALLET = (process.env.IMPACT_FUND_WALLET || '').trim();
const IMPACT_FUND_ENABLED = Boolean(IMPACT_FUND_WALLET);

// Percentage of revenue allocated to API operations (reduced to maximize owner share)
const API_OPS_RESERVE_PCT = Math.max(5, Math.min(40,
  Number(process.env.API_OPS_RESERVE_PCT || 10)));
...
const MIN_OWNER_PAYOUT_PCT = Math.max(25, Math.min(50,
  Number(process.env.MIN_OWNER_PAYOUT_PCT || 25)));
...
function allocateRevenue(revenueUsd, source = 'trading') {
  ...
  const allocation = {
    ...
    toOwner: 0,
    toImpactFund: 0,
    impactFundWallet: IMPACT_FUND_ENABLED ? IMPACT_FUND_WALLET : null,
    ts: Date.now(),
  };
  ...
  // Step 2: Impact Fund (10% by default — funds social / charitable causes via Solana)
  if (IMPACT_FUND_ENABLED && IMPACT_FUND_PCT > 0 && remaining > 0) {
    allocation.toImpactFund = Number((remaining * IMPACT_FUND_PCT / 100).toFixed(4));
    remaining -= allocation.toImpactFund;
  }

  // Step 3: API Operations Reserve (if below target)
  ...
  // Step 4: Gas Reserve (if below target)
  ...
  // Step 5: Everything else compounds into trading capital
  ...
  state.impactFundAccruedUsd = Number(((state.impactFundAccruedUsd || 0) + allocation.toImpactFund).toFixed(4));
  ...
  state.lifetimeToImpactFundUsd = Number(((state.lifetimeToImpactFundUsd || 0) + allocation.toImpactFund).toFixed(4));
