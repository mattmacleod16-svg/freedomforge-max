 ...
 *   - AI: Grok, OpenAI, Anthropic, Perplexity, Gemini, Groq, Mistral, Cerebras, NVIDIA, OpenRouter, HuggingFace
 ...
 *   5. AgentCard auto-top-up when API reserves run low (prepaid Visa)
 *   6. Report self-funding health status
 */
...
try { agentcardManager = require('./agentcard-manager'); } catch { agentcardManager = null; }
...
function runFundingCycle() {
  ...
  // Run AgentCard auto-top-up cycle if available and reserves are low
  let agentcardCycle = null;
  if (agentcardManager && allocationStatus.apiReserveDaysCovered < 7) {
    try {
      agentcardCycle = agentcardManager.runAutoTopupCycle();
    } catch (err) {
      console.error('[funding-coordinator] AgentCard cycle error:', err.message);
    }
  }
  ...
  state.healthHistory.push({
    ...
    agentcardActions: agentcardCycle?.actions?.length || 0,
  });
...
function getFundingStatus(availableModels = []) {
  ...
  return {
    ...
    // AgentCard (autonomous purchasing)
    agentcard: agentcardManager ? agentcardManager.getAgentCardStatus() : { available: false },
...
module.exports = {
  ...
  // AgentCard (autonomous purchasing)
  createAgentCard: agentcardManager ? agentcardManager.createCard : null,
  getAgentCardStatus: agentcardManager ? agentcardManager.getAgentCardStatus : null,
  runAgentCardTopup: agentcardManager ? agentcardManager.runAutoTopupCycle : null,
  approveAgentCardTopup: agentcardManager ? agentcardManager.approveTopup : null,
  denyAgentCardTopup: agentcardManager ? agentcardManager.denyTopup : null,
  recordCreditPurchase: agentcardManager ? agentcardManager.recordCreditPurchase : null,
};
