...
function deactivateKillSwitch() {
  save(KILL_SWITCH_FILE, { active: false, deactivatedAt: new Date().toISOString() });
  const state = loadRiskState();
...
function riskAdjustedSize({ baseUsd, confidence, edge, asset, venue }) {
  ...
  const state = loadRiskState();
  const today = new Date().toISOString().slice(0, 10);
  if (state.dailyPnl.date === today && state.dailyPnl.pnl < 0) {
    // Losing day — scale size inversely with losses
    const lossRatio = Math.abs(state.dailyPnl.pnl) / MAX_DAILY_LOSS_USD;
...
function recordPnl(pnl) {
  ...
  // Only trigger when peakEquity is meaningful (>$25) to avoid
  // false positives from small P&L fluctuations during early trading
  const MIN_EQUITY_FOR_KILL_SWITCH = 25;
