...
function riskAdjustedSize({ baseUsd, confidence, edge, asset, venue }) {
  ...
  // 5. Volatility-weighted sizing — reduce size for high-volatility assets
  if (asset) {
    ...
    if (HIGH_VOL_ASSETS.includes(assetUpper)) {
      size *= 0.5; // 50% size for high-vol altcoins
    } else if (MED_VOL_ASSETS.includes(assetUpper)) {
      size *= 0.75; // 75% size for medium-vol
    }
    // BTC and stablecoins get 100% (no adjustment)
  }

  // 6. Correlation-aware sizing — reduce size when correlated positions exist
  if (asset) {
    try {
      const correlated = getCorrelatedAssets(asset);
      if (correlated && correlated.length > 0) {
        const correlationPenalty = correlated.reduce((sum, c) => sum + (c.correlation || 0), 0);
        const corrFactor = Math.max(0.3, 1 - correlationPenalty * 0.4);
        size *= corrFactor;
      }
    } catch (e) { /* best-effort correlation check */ }
  }
