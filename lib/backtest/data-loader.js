...
try {
  ...
  log = createLogger('backtest-data-loader');
} catch {
...
async function fetchJson(url) {
  try {
    ...
    if (!res.ok) {
      log.error('HTTP error fetching URL', { status: res.status, url });
      return null;
    }
    ...
    log.error('Fetch failed', { reason, url });
    return null;
  }
}
...
function ensureCacheDir() {
  try {
    ...
    log.error('Failed to create cache dir', { error: err.message });
  }
}
...
function readCache(filePath) {
  try {
    ...
    log.error('Cache read failed', { file: path.basename(filePath), error: err.message });
    return null;
  }
}
...
function writeCache(filePath, candles) {
  try {
    ...
    log.error('Cache write failed', { file: path.basename(filePath), error: err.message });
  }
}
...
async function fetchBinanceCandles(symbol, interval, startMs, endMs, asset) {
  if (!BINANCE_INTERVALS.includes(interval)) {
    log.error('Unsupported Binance interval', { interval });
    return [];
  }
  ...
  while (currentStart < endMs) {
    ...
    log.debug('Fetching candles from Binance', { fetched: allCandles.length, estimated: estimatedTotal, asset });
...
async function fetchCoinbaseCandles(productId, interval, startMs, endMs, asset) {
  ...
  if (!granularity) {
    log.error('Unsupported Coinbase interval', { interval });
    return [];
  }
  ...
  while (currentStart < endMs) {
    ...
    log.debug('Fetching candles from Coinbase', { fetched: allCandles.length, estimated: estimatedTotal, asset });
...
async function fetchHistoricalCandles({ asset, interval, startDate, endDate }) {
  ...
  if (cached) {
    log.info('Cache hit', { file: path.basename(cachePath), candles: cached.length });
    return cached;
  }

  log.info('Fetching candles', { asset: assetUpper, interval, startDate, endDate });
  ...
  if (!candles || candles.length === 0) {
    log.info('Binance returned no data, falling back to Coinbase', { asset: assetUpper });
    candles = await fetchCoinbaseCandles(ids.coinbase, interval, startMs, endMs, assetUpper);
  }

  if (!candles || candles.length === 0) {
    log.warn('No candle data available', { asset: assetUpper, interval, startDate, endDate });
    return [];
  }
  ...
  log.info('Fetch completed', { candles: deduped.length, asset: assetUpper, interval, startDate, endDate });
...
function clearCache() {
  ...
  try {
    ...
    log.info('Cache cleared', { removed: result.removed });
  } catch (err) {
...
function getCacheInfo() {
  try {
    ...
    log.error('Failed to read cache info', { error: err.message });
    return [];
  }
}
