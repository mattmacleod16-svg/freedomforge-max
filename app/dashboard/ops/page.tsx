...
export default function OpsDashboardPage() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-[#030108] via-[#050110] to-[#030108] p-6">
      <div className="max-w-7xl mx-auto space-y-4">
        <div className="flex flex-col gap-2">
          <h1 className="text-3xl font-black text-amber-400 phoenix-title">📈 Ops Dashboard (Live)</h1>
          ...
          <a
            ...
            className="inline-block w-fit px-4 py-2 bg-purple-600 hover:bg-purple-700 rounded font-bold text-white"
          >
            Open Grafana in New Tab
          </a>
        </div>

        <div className="border border-purple-500/20 rounded-2xl bg-[#0d0619]/80 p-4 space-y-3">
          <div className="flex flex-wrap items-center gap-2">
            <button onClick={() => setPolicy('assisted')} disabled={loading} className="px-3 py-2 bg-[#1a0d2e] hover:bg-purple-900/60 border border-purple-500/20 rounded text-purple-200">Assisted</button>
            <button onClick={() => setPolicy('balanced')} disabled={loading} className="px-3 py-2 bg-[#1a0d2e] hover:bg-purple-900/60 border border-purple-500/20 rounded text-purple-200">Balanced</button>
            <button onClick={() => setPolicy('autonomous')} disabled={loading} className="px-3 py-2 bg-[#1a0d2e] hover:bg-purple-900/60 border border-purple-500/20 rounded text-purple-200">Autonomous</button>
            <button onClick={runGroundTruthSync} disabled={loading} className="px-3 py-2 bg-purple-700 hover:bg-purple-600 rounded text-white">Sync Ground Truth</button>
            <button onClick={runRetrainCheck} disabled={loading} className="px-3 py-2 bg-amber-700/80 hover:bg-amber-600/80 rounded text-amber-100">Retrain Check</button>
          </div>
          ...
          {statusMessage ? <div className="text-xs text-purple-300">{statusMessage}</div> : null}
        </div>

        <div className="border border-rose-500/20 rounded-2xl bg-[#0d0619]/80 p-4 space-y-2">
          ...
          <div className="text-sm text-purple-200/70 grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-1">
        ...
        <div className="border border-purple-500/20 rounded-2xl bg-[#0d0619]/80 p-4 space-y-2">
          <h2 className="text-lg font-bold text-purple-300">🔌 Protocol Integrations</h2>
          <div className="text-sm text-purple-200/70 grid grid-cols-1 md:grid-cols-3 gap-x-6 gap-y-1">
          ...
          <div className="space-y-1 text-sm text-purple-200/70">
            {(protocols?.protocols || []).map((protocol) => (
              <div key={protocol.protocol} className="flex flex-wrap items-center gap-2">
                <span className="inline-block rounded bg-purple-900/40 px-2 py-0.5 font-semibold text-white uppercase">{protocol.protocol}</span>
                <span className={protocol.enabled ? 'text-white' : 'text-purple-900/60'}>
                ...
                {protocol.latencyMs !== null && <span className="text-purple-300/50">({protocol.latencyMs}ms)</span>}
                <span className="text-purple-300/50">— {protocol.details}</span>
              </div>
            ))}
          </div>
        </div>

        <div className="border border-purple-500/20 rounded-2xl overflow-hidden bg-[#0d0619]/80">
          <iframe
